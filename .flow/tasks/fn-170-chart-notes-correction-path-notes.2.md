...
### Approach
- `_render_briefing_index_md` reads `chart.get('status')` at flowctl.py:17340 from the chart dict loaded at 17511, but the open->done flip happens at 17700-17740 AFTER the render call (17635). Compute the post-transition chart status first (or move the render after the transition block) and pass it in. Do NOT conflate with the `status` kwarg at 17325 - that is the BRIEFING's status.
...
**Required:**
- `plugins/flow-next/scripts/flowctl.py:17320-17360` - `_render_briefing_index_md` header + Notes copy
- `plugins/flow-next/scripts/flowctl.py:17511,17635,17700-17740` - load, render call, transition order
- `plugins/flow-next/tests/test_chart_briefing.py:1292` - notes-in-briefing fixture to extend

**Optional:**
- `plugins/flow-next/scripts/flowctl.py:27063-27093` - reopen + staled_briefings (fingerprint scope question)
