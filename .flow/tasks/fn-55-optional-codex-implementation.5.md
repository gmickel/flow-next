...
- **Receipts:** no generic `flowctl receipt` command exists — inline each delegated batch's result fields into the task's `flowctl done --evidence-json` (worker Phase 5, `worker.md:258-289`) as `evidence.delegation.result` (no scratch-file pointer). That IS the proof-of-work surface. <!-- Updated by plan-sync: fn-55.3 shifted Phase 5 done from worker.md:232-269 to 258-289 -->
...
- `plugins/flow-next/agents/worker.md:163-187` (Phase 4 `REVIEW_MODE=none` skip), `:258-289` (Phase 5 done + verification + `--evidence-json`) <!-- Updated by plan-sync: fn-55.3 inserted the Phase 2 delegation hook (worker.md L117-139), shifting Phase 4/5 anchors down (~+26 lines) -->
