...
## Description
Ship the experimental reply-prose skill (R7): trigger-scoped ambient application of docs/prose.md to substantial replies, per the experimental tier in agent_docs/adding-skills.md (steps 7/8/10 deferred; registry counts still bump).
...
- Guide: one experimental-variant line modeled on the work-rolling note (never a default route; name it only when the user asks for reply prose discipline; say it is experimental).
- prose.md line 3: adjust the exclusion sentence — replies are governed opportunistically via the experimental skill; the visual digest stays excluded.
- CHANGELOG: extend the existing Unreleased entry with one sentence, marked experimental, em-dash-free.
...
### Key context
- Experimental tier: NO conduct checklist, NO README/docs-site/mickel.tech count updates — adding those would make it an undocumented promise per adding-skills.md.
...
### Acceptance
- [ ] `skills/flow-next-prose/SKILL.md` + `commands/prose.md` ship with the experimental suffix, exclusion list, and pointer-not-payload body
- [ ] sync-codex openai.yaml entry + REQUIRED row added; `./scripts/sync-codex.sh` twice: idempotent, guards green, mirror committed
- [ ] Registry counts bumped in all three manifests; count-pin tests updated (registry side only, published counts untouched)
- [ ] Guide experimental note added; prose.md scope sentence adjusted; CHANGELOG Unreleased entry extended (em-dash-free)
