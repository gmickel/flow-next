...
3. **completionReview (fire-and-forget — R1, R10).** In `tracker-touchpoints.md` "Completion review" (:81-102, dispatch at :95) and phases.md 3g (:407-448, dispatch note at :443-445) change to fire-and-forget `Task flow-next:tracker-runner` (`operation: comment <spec-id>`, `event: completionReview`, `DISPATCH: forked`) — comment-shaped only, NEVER terminal Done (keep the fn-66 non-terminal doctrine verbatim). Record handle, continue.
...
7c. **completionReview event key — normalization SHIPPED in 2.9.1 (no work here beyond preservation).** The tag is now the top-level `completionReview` everywhere (phases.md, tracker-touchpoints.md, tracker-sync steps.md); `test_sync_check.py`'s key-parity class + prose guard enforce it. When rewiring the dispatch blocks in this task, KEEP `event: completionReview` — a reintroduced `work.` prefix fails the suite.
...
- [ ] tracker-touchpoints.md First-claim dispatch is an AWAITED `Task flow-next:tracker-runner` (push, event: work.firstClaim, DISPATCH: forked); Task-done + Completion-review are fire-and-forget runner dispatches (comment, event: work.done / completionReview).
