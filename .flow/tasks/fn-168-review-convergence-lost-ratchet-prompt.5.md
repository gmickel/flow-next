...
- **GUARD (non-optional — this is the reason the task is not just a config key):** `flowctl config set review.maxIterations` is a **self-grant path**. An autonomous agent could write itself 99 rounds durably into a tracked file, defeating fn-159's *"the implementing agent can never reset or extend its own gate."* `config set` is NOT currently on the ralph-guard blocklist (which blocks `reset-review-rounds`, `review-rounds reset`, and `--force` dispatches). Add it, with the same **tokenized** matching the existing screens use — memory `shell-command-allowlist-gates-must-2026-06-05`: tokenize, never substring-match. Follow the existing screens' shape, including the raw-text floor and the assignment-value smuggle patterns (`_RECOVERY_ASSIGN_RE` and friends, ~:702-738) and the argv pass (~:1340-1370).
...
### Key context
- Independent of `.1`/`.2`/`.3` in code, but it touches `flowctl.py` like all of them — serialize in one checkout unless isolated.
- The guard block is the load-bearing half of this task. A config key without it would *weaken* fn-159's invariant, making the spec net-negative on autonomy safety.
...
- [ ] `flowctl config set review.maxIterations` is BLOCKED by ralph-guard with tokenized matching (never substring), covering the raw-text floor and assignment-value smuggle shapes the existing screens cover; a guard test pins it
