...
- **Phase 0 in workflow.md:**
  ...
  - **Existing-PR refusal:** `gh pr view --json url,state,isDraft,number 2>/dev/null` — exit 0 + JSON = PR exists → refuse with hint to use `/flow-next:resolve-pr`. Exit 1 + "no pull requests found" = clean to proceed.
...
- [ ] `plugins/flow-next/skills/flow-next-make-pr/workflow.md` documents Phase 0 pre-flight: epic resolution, base detection cascade, branch-validity checks, existing-PR refusal via `gh pr view`, gh pre-flight (`command -v` + `gh auth status`).
