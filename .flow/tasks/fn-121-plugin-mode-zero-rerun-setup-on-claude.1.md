...
**Context**: Prototypes exist: `plugins/flow-next/bin/flowctl` (launcher) and `cmd_usage` in `plugins/flow-next/scripts/flowctl.py` (~L7534, registered near the `cat` subparser ~L29008). Probe evidence in session scratchpad binlab/FINDINGS.md. Read spec Architecture + Edge Cases first.

**Work**:
1. `bin/flowctl`: keep in lockstep with `scripts/flowctl` (fn-77 probe semantics: PYTHON_BIN, `py -3`, python3, python; intentional diff ONLY the exec target `$SCRIPT_DIR/../scripts/flowctl.py`). Follow the comment structure of the `LAUNCHER_SH` constant (flowctl.py ~L7326).
...
- Dual copies refreshed (`.flow/bin/flowctl.py`, `scripts/ralph/flowctl.py` byte-identical to `scripts/flowctl.py`).
