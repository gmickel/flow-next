...
- [ ] Real-install proof: capture a recursive listing (path, mtime, size) of the **installer-owned paths** under the primary home, run `CODEX_HOME=<tmpdir> ./scripts/install-codex.sh`, and assert that listing is byte-identical afterwards while `<tmpdir>` received the full surface. NOT a whole-`~/.codex` snapshot - that directory carries live Codex runtime state (logs SQLite + WAL, session rollouts, models cache) that churns independently of the installer and makes the comparison unrunnable unless Codex is quiesced (R1)
...
## Done summary
Codex installs now honor `CODEX_HOME`: `install-codex.sh` resolves `CODEX_DIR="${CODEX_HOME:-$HOME/.codex}"` (hooks.json cleanup and every operator-facing message included), and `sync-codex.sh` bakes the same runtime form at every skills/agents rewrite site - covering both the `$HOME/.codex` and the previously-missed `~/.codex` spelling for ralph-init templates and worktree-kit scripts, now correctly quoted. A new fail-closed guard rejects actionable primary-home references across the whole installed mirror surface outside a narrow narrative allowlist; the existing plugin-root guards were retargeted, not relaxed. Installer tests scrub inherited `CODEX_HOME` and gained alternate-home + spaces-in-path regressions.
## Evidence
- Commits: 559132586917f479c5c11aa3d8cf7230e23ee790, c859d06d3b9b0ba1f74850937d89277a5a5e1b1f, 1e6bcbd1fb657dc28253681b2ca88de13780aa1d
- Tests: baseline: green - cd plugins/flow-next/tests && python3 -m unittest test_precheck_mode_contract test_install_codex_legacy_cleanup test_codex_hooks_normalize -q (22 tests OK pre-edit), cd plugins/flow-next/tests && python3 -m unittest test_precheck_mode_contract test_install_codex_legacy_cleanup test_codex_hooks_normalize -q (25 tests OK), python3 scripts/run_tests_parallel.py (files=179 ran=3877 failures=0 errors=0 skipped=4), uvx ruff@0.16.0 check . (All checks passed), idempotency: ./scripts/sync-codex.sh x2; find plugins/flow-next/codex -type f -exec shasum {} + | sort | shasum equal across the second run (d235d96020103fa1c825e52e61c21d77a913d554). NOTE: the unsorted variant in the acceptance text is order-sensitive (ignored __pycache__/runs churn directory order) and yields different hashes for byte-identical content - sorted listing is the valid form, guard break proof A: reverted the ralph-init template rewrite to the ~/.codex literal -> sync exits 1 with 'executable primary-home reference escaped CODEX_HOME rewrite' x8; restored, guard break proof B: removed the two generic /skills/ rewrites and the plugin.json rewrite -> sync exits 1 with '2 unrewritten plugin-root /skills/ path refs' + 'Codex Plan manifest path must be ${CODEX_HOME:-$HOME/.codex}/plugin.json'; restored, real-install proof: 1146 installer-owned paths under ~/.codex (skills|agents|scripts|prompts|templates|references|plugin.json|config.toml) byte-identical (path+mtime+size) before and after CODEX_HOME=<tmpdir> ./scripts/install-codex.sh; <tmpdir> received the full surface and its flowctl resolves via ${CODEX_HOME:-$HOME/.codex}, spaces proof: CODEX_HOME='<tmp>/alt codex home' ./scripts/install-codex.sh exits 0 with the full surface, GREEN_RECEIPT: .flow/tmp/green-receipts/c859d06d-unittest.json
- PRs: