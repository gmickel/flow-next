...
## Done summary
Retroactive tidy (2026-08-04): the feature this task describes shipped long ago but the task record was never closed when the parent spec closed — surfaced by the closed-parent orphan rule in `flowctl brief` (3.15.0). Evidence of existence: glossary compounding is a live STRATEGY.md track fact: capture/interview write glossary entries, plan/work/review read them back (anchor bundles include the glossary index). No new work performed; this receipt closes the stale record.
## Evidence
- Commits:
- Tests:
- PRs: