...
- Name/read back Outcome, grounding, smallest visible breadth-first frontier, parked unknowns, and attended/unattended cost before persistence. Ask only questions not already answered by approved evidence; resolve nothing. No consequential unknowns means stop and recommend the smaller route.
- Chart mode renders the initial-map proposal and cost before persistence. If it exceeds the ceiling, offer narrowing/splitting first; pass `--force-size --reason` only after an explicit warning/read-back confirms the override. After persistence, close by offering parallel dispatch of the independent unattended frontier decisions as separate work invocations; the charting session itself still resolves nothing.
- Work mode: re-anchor Outcome and `## Notes` (this effort's working agreements and standing pointers); call `flowctl chart frontier`; choose one smallest uncertainty; claim; run the evidence route; resolve/scope/release; infer only newly-visible decisions; recompute. Never follow a frozen initial route.
...
- Prototype work creates or imports one scoped throwaway artefact, records it through `chart attach-asset` while the D-ID remains open, presents the exact safe reference/revision, and records the human reaction before resolve/supersede/sharpening. If the reaction does not happen, release normally with an awaiting-reaction note or leave crash state observable; a later invocation resumes from the existing artefact rather than rebuilding or inferring approval.
...
## Acceptance
- Chart mode names/read-backs Outcome, stops without persistence when no consequential unknown exists, otherwise creates only the visible frontier and cost estimate while resolving nothing, then closes by offering the independent unattended frontier for parallel dispatch as separate invocations.
