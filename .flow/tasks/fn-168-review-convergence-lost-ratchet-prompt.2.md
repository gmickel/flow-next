...
**Size:** M
**Files:** `plugins/flow-next/scripts/flowctl.py` (`_review_finding_prior_items`, and the prior-record regex family if the aggregate needs its own pattern), `plugins/flow-next/tests/test_review_findings_parser.py`, `plugins/flow-next/optimization/reached-path/fixtures/review-findings/v1/<backend>/*.md` + `INDEX.json` (all 6 backends), `plugins/flow-next/tests/test_review_findings_fixture_corpus.py`, `.flow/bin/flowctl.py` (propagation)

### Approach
- Parse `Prior findings: all fixed` in the SAME line-start family as the per-ordinal records, in one pass, so a malformed stray line stays a recognized-but-invalid signal (existing whole-container `None`) and can never be mistaken for a clean aggregate round.
...
- [ ] Fixture corpus case(s) added to `CASES` + `INDEX.json` for all 6 backends; matrix test green
