...
## Solution
Added a CLI-flag-then-env-var fallback in `cmd_next` at the time. (2026-07-25 correction: that cascade NO LONGER EXISTS in flowctl - the legacy 3.0.0 removals deleted `--epics-file`, `EPICS_FILE`, and `_emit_rename_deprecation`; `cmd_next` now reads only `--specs-file` / `SPECS_FILE`. The only surviving EPICS_FILE fallback is shell-side in `ralph-init/templates/ralph.sh` (`SPECS_FILE="${SPECS_FILE:-${EPICS_FILE:-}}"`). The Prevention lesson below - grep the spec for env-var acceptance before scoping to flags - remains the durable content of this Update.)
