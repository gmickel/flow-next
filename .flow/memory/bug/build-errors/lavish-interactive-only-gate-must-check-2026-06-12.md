...
## Solution
Commits 1235c6d + fce92a1: `LAVISH_OK` gate inside the snippet with BOTH checks — `[[ "${MODE:-interactive}" != "interactive" ]] && LAVISH_OK=false` (MODE comes from SKILL.md mode-detection) AND the env-marker family check; `lavish-axi` open + the poll instruction both live inside the guarded branch. Codex mirror regenerated. (2026-07-25: the gate now lives in `references/html-lens.md:48-53` after the §5.10 split-out; the env check was since generalized from a rigid var list to the marker *family* — a strengthening beyond what this entry records.)
