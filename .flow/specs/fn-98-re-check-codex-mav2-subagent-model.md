...
## Addendum 2026-08-07 (issue-state check only)

Upstream issue states re-checked via gh (codex latest release 0.147.0): #32782 CLOSED, #33268 CLOSED (both 07-16), #33314 OPEN, #33267 OPEN - consistent with R1's 08-03 probe-matrix recording. No new probes run today; the R1/R3 measured conclusions stand (steering works on both paths, verified from child rollout turn_context; sandbox_mode read-only is prompt-only). The remaining work is R2/R4-R9: the doc corrections that must say steering WORKS plus the read-only-guarantee rewrite. A 2026-08-07 session briefly overwrote this spec and pushed a contradicting docs refresh based on the stale Goal paragraph alone; both were restored/reverted the same day - the lesson (read the whole spec, Acceptance records supersede Goal prose) is in session memory.

## Closed 2026-09-14 (R2 delivered by fn-245)

R2's surfaces now state the measured facts instead of "steering is unreliable": the codex reach page in-host row (steering works on both paths since 0.146.0, the precedence rule, the two dispatch gotchas, `sandbox_mode` not enforced), the platforms.md worker-pin note, and the usage guide's self-bridge line. `orchestration.md` and the setup model-routing snippet carried no such caveat at close time. The flow-next.dev model-routing caveat is the site's half and lands in the fn-245 release walk. Delivered in spec `fn-245-bridge-child-owns-the-task-worker`, which also turns the "keep the child flat" caveats (openai/codex#33267) into dated watch lines.

R4 (the interview fact-scout pin on Codex hosts; the mirror still dispatches `agent_type: explorer` without a model) and R5 to R9 (the Codex read-only guarantee: the sync-codex.sh comment, the per-host guarantee in platforms.md, the CLAUDE.md parity bullet, the role-TOML key guard) remain out of scope of fn-245 and are not delivered by this close (the spec named R5 to R9 as out of scope; R4 is recorded here as undone rather than implied); the measured facts above stand for whoever picks them up.
