...
1. **Shared discipline reference `plugins/flow-next/references/tracker-dispatch.md` (NEW, <=50 lines)** - the single place the rules live; every touchpoint's dispatch line just points here (fn-82 progressive-disclosure shape, like `references/html-artifacts.md`). Content, in full:
   ...
   - **Host gate + degradation:** Claude Code only (cheap `CLAUDECODE` env probe, mirroring codex-delegation's pre-check); any other host, a missing runner, or a failed dispatch degrades to inline exactly as today - loudly, never silently.
...
5. **sync-codex plumbing:** the agents glob auto-generates `codex/agents/tracker-runner.toml` (loop at sync-codex.sh:1402-1437, confirmed glob); `sandbox_for()` (sync-codex.sh:101-107) gains `tracker-runner` -> `workspace-write`; a global `Task flow-next:tracker-runner` -> `tracker_runner` role rewrite + hard-fail guard (the fn-100 transform+guard pattern). The mirror renders the prose but its host gate resolves inline (Codex spawn_agent true-async unverified - fn-98 tracks; note the previously-cited "Codex mirror forbids background jobs" line is an RP-specific warning, NOT a general convention - the gate stands on unverified-async alone).
...
- Codex/Copilot/Droid/Cursor: inline in v1 (host gate). Cross-PROCESS serialization: out of scope (documented limitation).
...
- **R8:** Claude-Code-only host gate; Codex mirror gets the role rewrite + guard but resolves inline; sync-codex `sandbox_for` maps tracker-runner to workspace-write; full pytest + smoke green; CHANGELOG under `## Unreleased`, no version bump (batched releases).
