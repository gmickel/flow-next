...
- **R3:** Per-spec serialization for STATE-shaped ops only: at most one in-flight state-shaped op per spec id (a second awaits the first, and a state-shaped op awaits any in-flight op on its spec). Comment-shaped dispatches may overlap freely — even on the same spec — because they write only per-file receipts and never touch the spec JSON (validated 2026-07-05, see Feasibility validation). Different specs always run concurrently.
