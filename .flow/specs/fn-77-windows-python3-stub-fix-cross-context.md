...
- **Hook commands aren't guaranteed bash → but Ralph requires Git Bash anyway (plan-review + cursor-review Major, scoped):** `hooks.json` command strings may run under `/bin/sh` (no arrays/`source`). So `ralph-guard.py` is invoked via an install-produced bash wrapper (`scripts/ralph/hooks/ralph-guard`, bash shebang, sources the resolver) that `hooks.json` calls as `bash scripts/ralph/hooks/ralph-guard` — not inline `source`+array in the JSON. The wrapper handles `/bin/sh` but NOT native `cmd.exe`/PowerShell — which is fine: Ralph's harness (`ralph.sh`) is itself bash, so **Ralph mode requires Git Bash on Windows** regardless. Narrow + document that requirement rather than adding a native `ralph-guard.cmd` the harness couldn't use anyway. [paraphrase]
...
- **`scripts/ralph/` is install-produced, NOT committed (cursor-review Major):** there is NO committed `plugins/flow-next/scripts/ralph/*`. ralph-init CREATES `scripts/ralph/` in the *user's* project, copying `flowctl` + `flowctl.cmd` + `flowctl.py` + `pick-python.sh` into it. So committed launcher edits target only `plugins/flow-next/scripts/flowctl`(+`.cmd`) and this repo's dogfood `.flow/bin/flowctl`(+`.cmd`); the `scripts/ralph/` copies are exercised via the *installed* layout in tests, never edited as tracked files. [paraphrase]
- **Resolver path differs by layout (plan-review + cursor-review Major):** the shared resolver is `plugins/flow-next/scripts/lib/pick-python.sh`. The 11 plugin-tree smoke tests source `scripts/lib/pick-python.sh`. But `ralph.sh` and the ralph hook run from the INSTALLED `scripts/ralph/`, where ralph-init copied the resolver to `scripts/ralph/pick-python.sh` (flat — NOT `scripts/ralph/lib/`). So `ralph.sh` and the hook wrapper look up `$SCRIPT_DIR/pick-python.sh` (installed) with a source-tree fallback — they must NOT hard-code `scripts/lib/`, which doesn't exist in the installed layout. [paraphrase]
