...
- **R1:** In every interactive lifecycle skill carrying the `## Pre-check: Local setup version` block (interview, plan, work, capture, tracker-sync, sync, make-pr, resolve-pr, qa, prime, map, audit, prospect, strategy, memory-migrate, ralph-init, drive if present, etc. - enumerate by grep for the block at implementation time), a `setup_version` vs plugin-version mismatch with no matching `version_ack` triggers a blocking `AskUserQuestion` with exactly the options Refresh now / Remind me next version / Skip this run, before the skill proceeds.
