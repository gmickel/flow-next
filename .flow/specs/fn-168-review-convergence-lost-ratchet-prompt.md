...
- **R2:** A dedicated aggregate all-clear record `Prior findings: all fixed`, stated in the prompt and parsed in the same line-start family, marks every carried prior currently `open`/`not_fixed` as `fixed`. It must be recognized **without** forcing a RECORD/PRIOR count mismatch (verified live: today it matches the record regex and fails the canonical one, which would drop the container). Any per-ordinal record disables the aggregate path; `withdrawn` items are never swept; the path never fires on an empty prior set. **`unaddressed: []` alone is explicitly NOT a prior-findings signal — documented and negative-tested.** Absent both signals, today's conservative carry-forward stands. **Interim scope (added after PR #295 review): the sweep is withheld on a backend whose prompt has a hard size budget** — only cursor passes `max_total_chars`, so only cursor can render a partial prior set, and a reviewer shown a subset could truthfully answer the aggregate for what it saw while the sweep marked omitted findings `fixed` (a false SHIP). The line stays *recognized* there, so the container is never dropped; only the sweep is withheld, leaving priors carried at `open`. Per-ordinal records stay honored on every backend — they name the ordinal they resolve, so a partial view can only under-report. fn-169 removes the truncation itself and deletes this gate. Table-tested. Errors: aggregate record + a contradicting per-ordinal `not-fixed` → the explicit line wins; malformed stray line + aggregate record → recognized-but-invalid, never a silent all-clear.
...
- **The aggregate sweep is gated on cursor as an interim measure**, not a design position: the payload that can be truncated is a *duplicate* of what a resumed session already holds, and fn-169 deletes both the truncation and this gate. Do not build rendered-ordinal plumbing to prop it up.
