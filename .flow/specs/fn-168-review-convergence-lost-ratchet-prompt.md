...
3. **Classifier honesty guard (belt-and-braces).** `flat-trajectory` compares only evidence-bearing opens (fresh or explicitly `not_fixed`), excluding carried-unverified items from the count and severity inputs **on both sides of the comparison** — a presence gate does not fix the named regression, and a one-sided filter silently disables the stall class (see R3). `same-not-fixed-lineage` and `fresh-introduced-critical` semantics unchanged.
...
- **The advertised grammar must exist in the parser's vocabulary (plan-review round 1, P1).** Verified live: `_FINDINGS_PRIOR_RE` spells the negative status `not[\s_]fixed` — whitespace or underscore, **NOT a hyphen** — and `_FINDINGS_STATUS_ALIASES` has no `not-fixed` key. So a mandated `Prior finding #2: not-fixed` line matches the broad `_FINDINGS_PRIOR_RECORD_RE` but **fails** `_FINDINGS_PRIOR_RE` → RECORD/PRIOR count mismatch → `None` → the whole round's findings container is dropped. The same trap catches the aggregate record: `Prior findings: all fixed` matches RECORD_RE and fails PRIOR_RE today. **Every token the prompt advertises must be accepted by the canonical matcher and normalized by the alias table, and the aggregate record must not create a count mismatch** — otherwise this fix ships the exact failure class it exists to remove.
...
- **R1:** The ratchet/shrink-only prompt block states the exact per-ordinal line grammar with an example (line-start `Prior finding #N: fixed|not-fixed|withdrawn`, echoing the rendered ordinal), and **every status token the prompt advertises is accepted by `_FINDINGS_PRIOR_RE` and normalized by `_FINDINGS_STATUS_ALIASES`** — today's regex spells the negative `not[\s_]fixed` and rejects the hyphen, so hyphen support must land with the wording (or the wording must advertise only already-accepted tokens; the prompt and the parser must agree either way). A fixture-driven test proves a codex-style compliant response yields carried items with correct `fixed`/`not_fixed` statuses in the receipt findings container via the production parser. The same wording lands in all 3 `workflow-host.md` files. Errors: an out-of-vocabulary status word (e.g. `pending`) stays a recognized-but-invalid signal (existing whole-container `None`), never a silent absence; a single-item prior set without an ordinal still parses.
- **R2 (amended at plan time — see the R2 amendment above):** A dedicated aggregate all-clear record `Prior findings: all fixed`, stated in the prompt and parsed in the same line-start family, marks every carried prior currently `open`/`not_fixed` as `fixed`. **The aggregate line must be recognized by the same counting logic that guards unknown statuses** — verified live, it currently matches `_FINDINGS_PRIOR_RECORD_RE` but not `_FINDINGS_PRIOR_RE`, which would force the RECORD/PRIOR mismatch and drop the container; adding it must not create that mismatch. Any per-ordinal record disables the aggregate path (explicit beats implicit); `withdrawn` items are never swept; the path never fires on an empty prior set. `unaddressed: []` alone is explicitly NOT a prior-findings signal (documented, and negative-tested). Absent both signals, today's conservative carry-forward stands. Table-tested. Errors: aggregate record + a contradicting per-ordinal `not-fixed` → the explicit line wins; malformed stray line + aggregate record → recognized-but-invalid, never a silent all-clear.
- **R3 (predicate corrected at plan-review rounds 1-2 — P0/P1):** Classifier guard: `flat-trajectory`'s comparison runs over **evidence-bearing opens only** — fresh (`firstSeenThisRound == true`) or explicitly re-affirmed (`status == "not_fixed"`) — excluding carried-unverified items (`status == "open" && firstSeenThisRound == false`) from the count and worst-severity inputs. Two constraints are mandatory, not implementation judgment: (a) a mere **presence gate is forbidden** — the fn-158 shape contains one fresh finding, so a presence test passes and the 7-vs-6 comparison still stalls; (b) the filter applies **symmetrically to the previous and current digests** — filtering only the current side makes a genuinely flat fresh trajectory (1 fresh → 1 fresh) read as forever-decreasing against an unfiltered previous set, silently disabling the stall class. With symmetric filtering the fn-158 shape compares 1 evidence-bearing open against 6 → decreasing → no stall, while 1→1 still classifies. The fn-158 completion shape (6 open → 6 carried-unverified + 1 new) is the named regression fixture and must pass through to a normal round-3 reservation. Genuine stalls still classify: a re-affirmed `not_fixed` overlap, and equal-or-growing fresh open sets. Implemented as an inference over the existing digest row — no new digest/receipt field. Errors: a digest pair failing validation keeps today's behavior; `same-not-fixed-lineage` / `fresh-introduced-critical` unchanged.
- **R4:** End-to-end: a scripted two-round codex-style transcript (round 1 NEEDS_WORK with findings; round 2 NEEDS_WORK resolving all priors via the stated grammar + one new finding) reaches round 3 without ESCALATE under the default caps, driving the production reservation path. Plus the **three-round symmetry regression** (plan-review round 2): 6 fresh → 6 carried-unverified + 1 fresh → 7 carried-unverified + 1 fresh — round 3 is allowed, and the reservation following it MUST classify the flat 1→1 fresh trajectory as a stall. A second scripted transcript that genuinely stalls still ESCALATEs. No error surface beyond the assertions.
...
Corrections from plan-review round 1 (recorded because they contradict the plan as first written): (a) R3's original predicate — "require ≥1 re-affirmed or fresh open finding" — does NOT fix the named regression, because the fn-158 shape contains a fresh finding and the 7-vs-6 comparison still stalls; the comparison itself must exclude carried-unverified items. (b) The mandated `not-fixed` spelling and the `Prior findings: all fixed` aggregate are both rejected by `_FINDINGS_PRIOR_RE` today while matching the broad record regex — advertising them without widening the parser vocabulary would drop entire findings containers, reproducing the bug class. (c) The fixture corpus is rooted at repo-root `optimization/reached-path/`, not under `plugins/flow-next/`.

Round-2 corrections: (d) the previous-round comparison basis cannot be left to implementation judgment — an asymmetric filter (current filtered, previous not) makes a flat 1-fresh-per-round trajectory read as decreasing forever, so symmetry is a stated requirement with a three-round test; (e) task .1 originally advertised tokens whose parser support lived in the dependent task .2 — an impossible order — so the vocabulary/recognition work moved INTO .1 (its seam is now "everything the prompt advertises parses without dropping a container") while .2 keeps the aggregate's semantics, scoping, and corpus tests.
