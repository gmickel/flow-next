...
- **Mirror-transform coupling (silent failure class):** the closer literals are exactly the files the Codex sync rewrites with anchored per-pattern substitutions, and only one of the sixteen literals is covered by a hard-fail guard. Rewording a literal without updating its anchor leaves the mirror on the colon form at exit 0. A closer reword and its transform update are one atomic change.
...
- Do not edit impl-review's own prose: its `export` mode string is pinned by an existing prose-diet test, and no requirement here needs it changed.
