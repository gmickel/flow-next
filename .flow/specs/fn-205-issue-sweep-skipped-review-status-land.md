...
- **R1:** After a 3g policy skip, `completion_review_status` is a persisted value that is neither `unknown` nor `ship`, written by the skip itself. `completion_reviewed_at` means "when this status was written", never "a review ran" — `ship` remains the only claim that a review happened. Errors: the skip writes the value only from `unknown`, enforced atomically by the CLI (a compare-and-set guard on the setter, evaluated under the existing sidecar lock) rather than by a prose read-then-write, so a spec already carrying `needs_work` / `needs_human` is never silently upgraded to satisfied even if a review verdict lands between the skip's read and its write; a CLI rejection of the token surfaces as a stage failure, not a silent no-op. `[paraphrase]` `[strategy:Ralph autonomous mode]`
