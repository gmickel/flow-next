...
**Field evidence (2026-08-27, fn-205/fn-206 landing):** manual branch-on-branch stacking WITHOUT the native stack API measured its failure modes live, and they are the negative image of what this spec buys. Squash-merging the base with `--delete-branch` made GitHub close the dependent PR permanently (reopen and retarget both refused on a closed PR whose base is gone — no auto-retarget fired); the dependent branch then needed a rebase onto main (conflicts only in generated files), a force-push, and a successor PR (#374 superseding #373); and the squash orphaned every SHA the stack's receipts recorded (task evidence commits, rebaseline baselines — all flagged by codex post-merge). Recovery playbook + avoidance notes: `.flow/memory/knowledge/workflow/stacked-pr-squash-close-recovery-2026-08-27.md`. Until this spec and fn-149 land, treat manual stacks as land-incompatible: land the base first, then branch the dependent off main. `[plan]`
