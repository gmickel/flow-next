...
- Post-capture drift (2026-08-09, 3.18-3.21): the setup workflow now carries Done-when bounds, and the Claude detection signal is named by more than one of its prose surfaces (cascade branch, bound, detection-fixtures matrix). Invariant for #306: every surface naming the signal changes in the same edit - a workflow that keys the branch on one signal while a bound or matrix still names another has broken this. `CLAUDECODE=1` is inherited by child processes (same class as the documented `CURSOR_AGENT` misfire), so the branch pairs it with a positive discriminator per the workflow's own inherited-env doctrine. If flow-98 (packaged-delegation removal) lands first, the guard cited above as the idiom's prior art disappears; the idiom remains valid without it. Concrete anchors: task 2.
