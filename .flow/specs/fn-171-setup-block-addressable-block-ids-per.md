...
All in the setup-block section of `plugins/flow-next/scripts/flowctl.py` (2543-2841; argparse at 47203-47223 post-fn-170 - anchor by symbol, not line), plus dual-copy propagation, tracker-manifest regen, and codex mirror regen.

1. **Block id (a).** Optional `--id <BLOCK-ID>` on `apply`, `resolve`, and the new `check`. Default `FLOW-NEXT` derives exactly the current markers - zero behavior change for the two existing callers (setup skill workflow.md:828, 841-842). An explicit `--id FLOW-NEXT` is byte-identical to omitting the flag (normalized before keying - never a distinct state entry). Id validation (reject, never sanitize): non-empty, max 64 chars, charset `[A-Z0-9][A-Z0-9._-]*`, and NO `--` substring (HTML comments cannot contain `--`; a bad id would make its own marker line ambiguous). Validation runs in the command body via `error_exit` (exit 1), NEVER as an argparse `type=` validator (argparse usage errors exit 2, colliding with check's drift code) - and before any file read. Custom id derives `<!-- BEGIN <ID> -->` / `<!-- END <ID> -->`.
   **Template↔id consistency (required):** templates carry their own marker lines and apply writes the template verbatim, so `apply`/`resolve`/`check` MUST fail exit 1 ("template does not contain the marker pair for id <ID>") when the template does not contain exactly one standalone derived pair for the operated id. Without this, `apply --id DEPLOY` with a FLOW-NEXT-marked template appends an untracked FLOW-NEXT block and re-appends forever (no DEPLOY span is ever found). Fixture required.
...
4. **Read-only `check` (c).** `flowctl setup-block check --file <target> --template <canonical> [--id <ID>] [--json]`. Computes the same classification `apply` computes - byte-equality FIRST, same as apply: a byte-pristine block is `unchanged`/exit 0 even when the recorded hash carries the `customized` sentinel. Zero writes: no target rewrite, no meta.json mutation, no hash re-record. It acquires the setup-block lock briefly for a consistent meta read (no mutation under it). Verdicts and exit codes:
   ...
   The drift/structural verdict strings reuse apply's transition vocabulary where the states coincide (`unchanged`, `customized`, `hash-absent`) and name the read-only-only states (`missing-file`, `missing-markers`, `template-drift`, `corrupt`) - apply expresses those as actions or hard errors, so "no new synonyms" applies to shared states only. JSON reuses the `_setup_block_emit` shape (`target`/`action`/`reason`/`hash`, action = verdict); no `command` field exists in setup-block emissions (verified against `json_output` + `_setup_block_emit`) and none is added - the verdict vocabulary alone disambiguates check output. Docs note: argparse usage errors also exit 2, so CI recipes that must distinguish drift from usage errors key on the JSON verdict, not the bare exit code.
...
- Docs: `docs/flowctl.md` setup-block section (152-165) documents `--id`, multi-block state, and `check` incl. the CI recipe for copy-mode repos and the exit-code table. `agent_docs/setup-modes.md` invariants: REWRITE invariant 2 itself ("Setup-block markers are EXACT strings... Never parameterize the marker") to the new truth - markers are derived deterministically from the block id, the default id yields the historical exact strings, free-form marker overrides remain forbidden - plus the per-(path,id) keying note. Appending a note while leaving the old invariant standing ships a self-contradicting review-blocker list.
...
- Template↔id mismatch: `apply --id DEPLOY` with a template carrying only the FLOW-NEXT pair -> exit 1, no write, error names the missing derived pair (prevents the infinite re-append + untracked-block failure mode). Same validation on resolve and check.
...
- R1: `apply`/`resolve`/`check` accept `--id`; omitted id and explicit `--id FLOW-NEXT` behave byte-for-byte as today in OBSERVABLE CLI behavior (target-file bytes, markers, transitions, exit codes, JSON emissions). Existing tests stay green with at most their meta-SHAPE assertions updated to the nested form (Architecture item 3) - no behavioral assertion changes. Errors: invalid id (charset, length, `--` substring, empty) rejected via `error_exit` (exit 1) with a clear message before any file read - never argparse-level (exit 2).
- R2: A custom id operates on its own derived marker pair; content outside that pair - including other ids' managed spans and stray markers belonging to other ids - is byte-preserved and never triggers fail-close for the operated id. Errors: unpaired/embedded marker FOR THE OPERATED ID fails closed, no write; a template lacking exactly one standalone derived pair for the operated id fails exit 1 before any target write (template↔id consistency, Architecture item 1).
