...
Who feels it: whoever is waiting. The unit suite's two slowest files spend most of their wall inside flowctl spawns (one does 181 spawns for 69 tests, another 168 for 53), and suite wall is set by the slowest single file, so the local gate, every worker wave, and every autonomous tick inherit the tax. Users feel it too — in plugin mode flowctl is PATH-resolved through the bundled wrapper, so the same recompile happens on their machines.

This is an **iteration-speed** change, not an agent-speed change: model generation dominates end-to-end wall, and per-call ceremony has been measured at effectively zero against it. No claim beyond the startup number belongs in this spec or its changelog entry.
...
- **R1:** The main CLI path executes from cached bytecode, and warm startup for a representative read-only command improves by **at least 2x** versus the same command on the pre-change tree, with the measurement method and before/after numbers recorded in the task summary and the PR. Errors: unwritable directory → pyc write skipped, command succeeds unaccelerated; stale pyc → standard CPython invalidation; a distribution channel missing the entry → actionable message naming the entry and remedy, non-zero exit, never a traceback.
