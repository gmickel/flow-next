...
- **R7:** The unit suite's own flowctl invocations route through the entry, so the local gate actually collects the saving — the suite resolves flowctl through a per-file constant today and therefore keeps paying the compile cost even after the wrapper is fixed. Before/after wall recorded for the two spawn-heaviest test files. Errors: a test that must invoke the source script directly (bootstrap or launcher-parity coverage) keeps doing so and is named as a deliberate exception.
