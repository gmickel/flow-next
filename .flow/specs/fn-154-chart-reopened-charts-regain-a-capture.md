...
- **R3:** `--json` envelopes for the new path carry an explicit outcome discriminator, and per-briefing `status` remains the single source of truth for capture-readiness with no consumer-side inference needed. **Wording corrected during completion review (2026-08-01):** this criterion originally named `chart show --json` as that surface. It is not - `compact_chart_metadata` projects `briefable` and `briefing_count` only, and has since fn-135. The statuses live in the chart record's `briefings[]`, which is where a consumer reads them and where the tests pin them. The requirement is unchanged in substance (an authoritative status a consumer can read directly); only the surface it names is corrected. Adding a per-briefing projection to `chart show --json` was deliberately NOT done: it is new public surface beyond this fix, and task .2's shipped acceptance requires that projection stay unchanged.
