...
1. **`chart_decision_revision`** (`flowctl.py:14212-14260`) - the blob gains the chart's reopen epoch. **The key is omitted entirely when the chart has never been reopened**, so every chart on disk today hashes byte-identically and its stored B-IDs keep matching. This is what keeps R4's "normal idempotence byte-unchanged" true across the upgrade rather than only for charts created after it.
...
**Naming constraint.** `Outcome` is already a load-bearing chart term (the map's stated goal, the `## Outcome` heading, `chart create --outcome`). The new discriminator MUST NOT be called `outcome`; pick a name in the emission-result vocabulary (`emission`, `supersedes_stale`, or similar) and use it consistently across envelope, docs, and tests.
...
- **R8:** The fix is state-source-agnostic and repeatable: a reopen sourced from `abandoned` behaves identically to one sourced from `done`, and a second reopen-then-rebrief round trip mints again rather than re-matching. Both proven by tests.
- **R9:** The `--json` discriminator is **not** named `outcome` (the term is already bound to the chart's stated goal - `chart create --outcome`, the `## Outcome` heading). Its name and value set are documented in `docs/flowctl.md` alongside the envelope error classes and pinned by an assertion in `test_chart_docs_inventory.py`, so the doc cannot silently drift from the code.

## Early proof point

Task fn-154-chart-reopened-charts-regain-a-capture.1 validates the core approach: that folding the reopen epoch into the fingerprint fixes the repro **while keeping every existing idempotence pin green** (`TestFingerprintVersioning`, `TestEvidenceFingerprint`, `TestVersionedBriefingPaths`, `TestDoneAndMutations`, `TestForcedDraft`, `TestBriefingFailpoint`).
...
| R4 | `--force` and normal idempotence byte-unchanged | .1, .3 | - |
...
| R9 | Discriminator not named `outcome`; documented and pinned | .3 | - |

## References

- Bug site: `plugins/flow-next/scripts/flowctl.py:14899-14913` (idempotent-return branch), `14915-14922` (done/abandoned guard)
- Fingerprint: `chart_decision_revision` `14212-14260`, `_briefing_fingerprint` `14263-14278`, `_briefing_evidence_digest` `14281-14295`
...
- fn-135 constraints: R41 (immutable/versioned briefings), R45 (identical fingerprint reuses a B-ID), R23 (ids never reused)
