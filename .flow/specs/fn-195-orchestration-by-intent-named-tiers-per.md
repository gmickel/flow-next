...
**Express intent, not mechanism — that is what makes this future-proof.** Mechanisms churn constantly: spawn primitives get renamed, CLI flags change, model identifiers rotate, new harnesses appear. Intent does not. A tier and a reach description survive all of that because the orchestrator resolves them with whatever the host offers *that day*; an enumeration has to be edited every time any of it moves. Every place this spec chooses prose over machinery is that same bet, and the bet is the reason the design gets simpler as the ecosystem gets messier rather than the other way round.

**The routing block is the durable form of an ad-hoc prompt.** A consumer who today writes their routing preference at the start of every session can instead write it once in their instruction file and stop repeating it — same words, same effect, read every turn. Neither path is privileged: an explicit instruction in the moment still wins over the standing block, which is exactly the precedence order stated at every dispatch site. The block removes repetition, not control.
