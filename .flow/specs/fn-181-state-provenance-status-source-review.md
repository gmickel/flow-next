...
- Post-capture drift (2026-08-09): skill routers no longer carry judgment prose (route-don't-carry), so the reviewer-visible home for the R2 sentences is each review skill's shared prompt template - the prose every backend actually sends. Those templates are pinned by `test_prompt_text_pinned` and byte-mirrored as the `PLAN_REVIEW_PROMPT_FALLBACK` / `COMPLETION_REVIEW_PROMPT_FALLBACK` constants (guarded by `test_review_prompt_template_parity`), so R2's edit is a prompt change: hash-pin updates + fallback parity + dual copies + mirror, with the prompt rationale stated in the commit message. Concrete anchors: task 2.
