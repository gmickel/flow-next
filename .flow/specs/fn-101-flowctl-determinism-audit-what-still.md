...
Remaining 3.0-deferred item: only the Ralph extract-or-sunset decision (section 3). Migrations + aliases moved into fn-111 per maintainer decision 2026-07-19. Droid hook facts re-verified 2026-07-19 (Factory hooks reference): shell tool is still `Execute` (no `Bash`), file tools are `Edit`/`Create`/`ApplyPatch` (no `Write`) - the dual matcher stays needed and the guard body must honor it (fn-114). Hooks inventory confirmed: ralph-guard is the plugin's ONLY hook (all four events); flow-next has zero non-Ralph hooks.
