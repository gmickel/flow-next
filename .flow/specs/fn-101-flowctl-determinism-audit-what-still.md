...
|---|---|---|---|
| 1 | Triage LLM judge: flowctl shells `codex exec --model gpt-5-mini` to answer "is this diff worth a full review?" | flowctl.py:26991-27138 (~245 LOC) | The doctrine's named symptom verbatim ("spawning codex to make judgments"). The deterministic whitelist rung stays; the ambiguous case should return AMBIGUOUS to the host skill, which is an LLM already. |
