...
- **R1:** The `RP_ELIGIBLE` guard (identical predicate to fn-78) is computed in canonical `flow-next-impl-review/SKILL.md` and `flow-next-spec-completion-review/SKILL.md` before their backend-guidance text renders.
- **R2:** When `RP_ELIGIBLE=0`, both skills' user-facing steering — Backends summary, "Backend at a glance" (rp/"Primary backend" line), ASK-error message, and `--review=…` override-hint echoes (incl. `workflow-common.md:35,39` and the `echo "Review backend: …"` line) — omits rp and lists only `codex`/`copilot`/`cursor` (+ `none`).
