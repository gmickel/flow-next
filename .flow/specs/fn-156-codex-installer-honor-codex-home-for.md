...
2. **`sync-codex.sh`** - the rewrite sites currently emit the literal into generated artifacts. They must emit the runtime-evaluated form instead. Verified inventory of what the mirror carries today (57 occurrences across 46 files):
   ...
   All are executable path references inside bash blocks, not narrative prose, so a shell parameter expansion is evaluated rather than displayed. That is what makes the substitution safe as a blanket rewrite.
...
- **R2:** Installed skills and agents carry no baked `$HOME/.codex` literal (`grep` clean across `plugins/flow-next/codex/`); they resolve flowctl and sibling scripts through `${CODEX_HOME:-$HOME/.codex}` at runtime. The `sync-codex.sh` guards at `:1848`, `:1860-1864`, `:1984` are retargeted to the new form and still fail on an unrewritten plugin-root reference - proven by a deliberate temporary break that the guard catches.
- **R3:** `README.md` documents the per-home install (`CODEX_HOME=<home> ./scripts/install-codex.sh`, run once per home).
- **R4:** `./scripts/sync-codex.sh` run twice is byte-idempotent (`git diff --stat plugins/flow-next/codex/` empty after the second run), and `test_precheck_mode_contract.py` plus the existing Codex parity and no-drift tests are green - updated where they pin the old literal, never relaxed to pass.
