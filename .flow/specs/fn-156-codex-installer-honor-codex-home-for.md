...
find plugins/flow-next/codex -type f -not -path '*/__pycache__/*' -exec shasum {} + | sort | shasum
./scripts/sync-codex.sh
find plugins/flow-next/codex -type f -not -path '*/__pycache__/*' -exec shasum {} + | sort | shasum   # must match
```

The `sort` is load-bearing and was missing from the first version of this check: bare `find` output is directory-order dependent, so the unsorted form produced three different hashes for byte-identical content during implementation. The `__pycache__` prune is for the same reason - transient churn that is not part of the mirror.
...
- **R1:** `CODEX_HOME=<tmpdir> ./scripts/install-codex.sh` installs entirely into `<tmpdir>` and writes nothing to `~/.codex`; with `CODEX_HOME` unset the script behaves exactly as today. Proven, not asserted - and scoped so the proof is actually runnable. Capture a recursive listing (path, mtime, size) of the **installer-owned paths** under the primary home before and after the redirected install and require it byte-identical. It must NOT be a whole-`~/.codex` snapshot: that directory also holds live Codex runtime state - logs SQLite and its WAL, session rollouts, the models cache, computer-use config - which changes continuously while Codex is running, so a whole-home comparison fails for reasons unrelated to the installer and can only pass with Codex fully quiesced. Scoping to installer-owned paths is what actually demonstrates the property under test: the installer wrote nothing to the primary home.
...
- **R3:** `README.md` documents the per-home install (`CODEX_HOME="$HOME/.codex-work" ./scripts/install-codex.sh`, run once per home). The documented example must be **copy-paste safe**: an angle-bracket placeholder like `<home>` is parsed by bash as redirections, so pasting it reads from a file named `home` and TRUNCATES the installer to zero bytes..
- **R4:** `./scripts/sync-codex.sh` is byte-idempotent **across the second run**, proven by a check that works while the first run legitimately rewrites ~46 tracked files: hash the mirror after run 1, run it again, hash again, require the two hashes equal. The hash must be **order-independent** (`find ... -not -path '*/__pycache__/*' -exec shasum {} + | sort | shasum`) - bare `find` output is directory-order dependent and yields different hashes for byte-identical content, which was observed in practice on this task. `git diff --stat` against HEAD cannot serve here - it reports the intended first-run regeneration and so can never be empty during this task, which made the original acceptance unsatisfiable. `test_precheck_mode_contract.py` and the existing Codex parity and no-drift tests stay green, updated where they pin the old literal, never relaxed to pass.
