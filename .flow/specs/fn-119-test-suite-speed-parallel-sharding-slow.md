...
6. **Scoped verification convention (repo-specific)**: this repo's specs stop listing the full discover as a per-task Quick command. New convention: Quick commands list the FOCUSED suites for the feature's files (e.g. `python3 -m unittest test_config_snapshot test_task_create_files -q`) - that is what workers baseline against and verify per task; the FULL (parallel, item 1) suite runs ONCE at the final gate (work Phase 4 / completion review), where gate receipts already dedupe it. Update this repo's .flow spec template copy + usage.md guidance accordingly. The worker/gate mechanics need no product change - workers already run "the spec's Quick commands"; this fixes what our specs put there.
