...
- **R12:** SKILL.md gains the Edit D subsection verbatim (async fact-scouts in rounds mode, including the portable-host dispatch clause for Cursor-style hosts without an Explore builtin); Codex mirror regenerated - sync-codex rewrites the Explore dispatch to `spawn_agent`/`explorer` AND the Claude-specific tier example "(sonnet on Claude Code)" to "(the host's mid-tier)", both hard-fail guarded; one sentence added to the flow-next.dev interview page's Question rounds section (flow-next.dev commit, separate repo, build green); CHANGELOG Unreleased entry extended with one bullet; `optimization/interview/changelog.md` Experiment 3 entry gains an "Async fact-scout addendum" paragraph (data in the Appendix); no results.tsv row (feature validation, not a prose mutation of the emission harness). Cursor host note: the installers blanket-copy the plugin tree, so no install-script enumeration exists to update; the canonical files are consumed as-is on Cursor.
- **R8:** `references/doc-aware.md` "per interview turn" redefined at all six sites, split by kind (amended during task .1 review round 1): the four THROTTLE/CADENCE sites (~lines 55, 76, 131, 171) become per-ROUND - one glossary question per round, per-round glossary re-read note, one decision write per round, one strategy-conflict question per round with the combined doc-aware budget staying "3 max" per round; the two behavior-(b) fuzzy-term TRIGGER sites (~lines 63, 78) become observation-based REPLY counts ("<count> replies", "<=6 user replies") - NOT rounds, because a full rounds interview is only 3-5 rounds and a rounds-based trigger threshold would make fuzzy-term sharpening unreachable. Intent preserved: meta-questions never crowd a round; the budget does NOT multiply by calls within a round; triggers keep firing at the same observed-usage frequency as before.
...
| R9 | Edit C: R-ID references glossed in question bodies (gist, not quote) | fn-100-interview-question-rounds-frontier.1 | - |
