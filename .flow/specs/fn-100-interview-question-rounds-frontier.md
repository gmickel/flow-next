...
Like rule 6, this is an additive post-eval bullet (legibility, not question-selection prose); it extends the contract's existing no-unexplained-shorthand rule to R-IDs.
...
- **R9:** SKILL.md plain-language question contract gains the Edit C bullet, verbatim: questions citing a spec R-ID inline the criterion text in full at first mention ("R1 - <criterion text>"), never a bare R-ID pointer that forces the interviewee to open the spec.
...
- **R8:** `references/doc-aware.md` "per interview turn" redefined as "per round" at the four genuine throttle/cadence sites (lines ~55, 76, 131, 171): one glossary question per round; the glossary re-read note reads per-round; one decision write per round; one strategy-conflict question per round with the combined doc-aware budget staying "3 max" per round. The two behavior-(b) TRIGGER sites (~63, 78) stay observation-based instead of round-based - the sharpening body says `<count> replies` and the short-conversation skip heuristic becomes "<=6 user replies" - because a full rounds interview is only 3-5 rounds, so a "<=6 rounds" threshold would have made fuzzy-term sharpening unreachable (impl-review r1 correction, amended from the original "all six sites per-round" wording). Intent preserved: meta-questions never crowd a round; the budget does NOT multiply by calls within a round; behavior (b) stays reachable in normal interviews.
