...
- **R3:** Triage gates eligibility on the **explicit readiness signal** — the fn-58 flow ready gate set OR tracker status at/beyond `tracker.readyState` — never on an inferred completeness score; items lacking the signal are left for the human to promote. It classifies and **routes**: workable → run the span; promoted-but-thin → `ask` (kick back, don't build); needs-spec / blocked / needs-human → ask or sequence. It never authors a spec.
