...
- **Tier C — small skills** (<~6k always-loaded: `deps`, `sync`, `map`, `prime`, `memory-migrate`, `setup`, `ralph-init`, `drive`, `export-context`, `rp-explorer`, `worktree-kit`, `flow-next` core): loop overhead exceeds any possible win. One time-boxed manual pass each, reviewed, no harness.
