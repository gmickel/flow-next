...
6. **Eval harness home**: `agent_docs/guidance-eval/` (runner script + scenario prompts + README with the grading contract), following the tracker-sync-spikes dev-archive precedent. Claude cells run with the probe-verified clean-room mechanism (default config dir + `--setting-sources project,local` - see R4); codex cells use explicit `--sandbox`/`--skip-git-repo-check`. Results recorded in the README's ledger table, optimization-log style. Not CI-wired (dev tool).
...
- **R4:** The guidance eval is extended with a multi-task/deps/reset scenario and re-run green on the trimmed docs (minimal arm; sonnet + terra + haiku floor) BEFORE the usage.md trim merges; harness committed as a repeatable script under agent_docs/guidance-eval/ with a verified clean-room mechanism for claude cells: default config dir + `--setting-sources project,local` (suppresses user-level CLAUDE.md/settings while preserving the OAuth login), auth- and leak-probed before use. (Probe-rejected alternatives, transcripts in the harness README: `--bare` is API-key-only - it disables OAuth entirely - so it is the documented alternative only when an ANTHROPIC_API_KEY is available; a fresh per-run `CLAUDE_CONFIG_DIR` drops the login too, since auth state lives in the config dir.)
...
- Eval methodology conventions (2026-07-14/15, amended during .2): clean-room bridge agents (claude cells via default-config-dir `--setting-sources project,local` isolation, auth- and leak-probed - `--bare` is incompatible with OAuth-only environments and a fresh `CLAUDE_CONFIG_DIR` drops the login; `codex exec` explicit sandbox), >=3 reps on discriminating cells, deterministic state grading, weak-model floor arm, scratch dirs under the harness's own tmp.
- Baseline finding (.2, drives a block amendment in .3): the dieted block scored 9/14 vs the minimal arm's 14/14 - weak tiers (haiku slugify 0/3, sonnet multitask 1/3) complete `done` with schema-SHAPED but EMPTY evidence lists. Placeholder style and the typical-flow line measurably change compliance; .3 adjusts the block's evidence line toward the minimal arm's demonstrated phrasing (lockstep + budget tests re-run; .4's gate is the verdict).
