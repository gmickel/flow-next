...
- **R4:** The guidance eval is extended with a multi-task/deps/reset scenario and re-run green on the trimmed docs (minimal arm; sonnet + terra + haiku floor) BEFORE the usage.md trim merges; harness committed as a repeatable script under agent_docs/guidance-eval/ with a verified clean-room mechanism for claude cells: isolated per-run `CLAUDE_CONFIG_DIR` (suppresses global CLAUDE.md/hooks/plugins while preserving account-level OAuth), leak-probed before use. (`--bare` was the original binding but is API-key-only - it disables OAuth entirely - so it is the documented alternative when an ANTHROPIC_API_KEY is available, not the requirement.)
...
- Eval methodology conventions (2026-07-14/15, amended during .2): clean-room bridge agents (claude cells via isolated `CLAUDE_CONFIG_DIR` with auth + leak probes - `--bare` is incompatible with OAuth-only environments; `codex exec` explicit sandbox), >=3 reps on discriminating cells, deterministic state grading, weak-model floor arm, scratch dirs under the harness's own tmp.
