...
### Record of passes and the build list (as of 2026-09-17, after pass 5)
...
5. Third scout on the unstable route samples, the tiny-vs-build boundary, and harness/model tier routing scored against three-scout judgment.

Six to build, each opt-in behind the key with byte-identical behaviour when the key is absent:
...
6. Tier routing at the worker's implementer resolution and scout dispatch: confident mechanical (Choice confidence at or above 0.8) goes to the cheap tier, confident long-running is a bridge recommendation, the moderate/intelligent boundary stays with the host; gated on a ten-task cheap-tier SHIP counterfactual.

Route-assist numbers of record (held-out third, floor 0.7, after the third scout): 85% coverage at 95% covered accuracy, with lifecycle kinds masked on non-live views.
