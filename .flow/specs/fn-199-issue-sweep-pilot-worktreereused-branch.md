...
- MERGED PR(s) exist, spec still open, no OPEN PR: check for unshipped work, `git rev-list --count <default-base>..<branch-head>` (or equivalently: refuse only when the newest merged PR's head equals the current branch head). Non-zero unshipped commits: NOT an inconsistency; classify the spec as make-pr-eligible and proceed. Zero: keep `NEEDS_HUMAN` (a merged PR with nothing new and an open spec is the genuinely inconsistent state). rev-list failure (unknown base): keep `NEEDS_HUMAN`, unchanged from today.
