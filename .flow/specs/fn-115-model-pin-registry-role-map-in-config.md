...
5. **Role map seeds**: fastJudge = luna-class (codex: gpt-5.6-luna; copilot: claude-haiku-4.5; cursor: composer-2.5 or gpt-5.6-luna-low). Scout roles map ONLY the codex-mirror translation layer: Claude-side scout tiers are family aliases (8 haiku prime-scanners, 11 sonnet judgment-scouts, opus quality-auditor, inherit worker/resolver - host-resolved, never rot, out of scope); sync-codex.sh's INTELLIGENT=gpt-5.5 / FAST=gpt-5.4-mini pins are what scoutIntelligent/scoutFast replace, with scoutFast moving off the never-A/B-tested gpt-5.4-mini to a luna tier at first refresh.
6. **No minis anywhere** (maintainer 2026-07-19): mini/nano-class models never appear as defaults, role seeds, or review-ladder rungs - luna and terra are the fast/cheap tiers of choice. Sole exemption: pure availability probes where output is discarded (e.g. the rp deep-pass probe_model at flowctl.py:21160), where cheapest-possible is correct.
7. **Review default effort: sol:medium** (maintainer decision 2026-07-20, eval-backed): a 3-model/3-tier blind-judged reviewer eval showed gpt-5.6-sol:medium within 2pts of sol:high on the only discriminating language (92 vs 94 JS defect detection) at 31% fewer tokens and lower wall-clock; Luna xHigh ties medium at +49% tokens/+54% wall; Terra misses bugs (78% JS, 67% Go) and is NEVER a reviewer. Seed the `review` role at codex:gpt-5.6-sol:medium with sol:high as the documented escalation tier (per-task `review:` pins); update the registry default_effort and the setup-scaffolded CLAUDE.md routing text accordingly. Already applied as repo config (review.backend codex:gpt-5.6-sol:medium, 2026-07-20).
8. **Gemini excluded for now** (maintainer 2026-07-19): do not seed Gemini rows into any ladder or role despite copilot offering them; revisit only on explicit request.
