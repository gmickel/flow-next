...
- **LEG6 Atomic file-pair rule:** detect designer/generator paired files where editing one side without the other corrupts the project, and require a pairing rule in the agent file. *(Instances: Delphi .dfm+.pas, WinForms .Designer.cs+.cs, Qt .ui+, Xcode storyboard+swift, protobuf generated+source.)*
- **LEG7 Tool-managed never-edit list:** files an IDE/toolchain rewrites or that are semantically opaque get an explicit never-touch list in the agent file. *(Instances: .dproj/.dsk/.identcache/__history, xcodeproj internals, .sln user files, .idea/.)*
