...
| plan-review | `skills/flow-next-plan-review/SKILL.md:50-56` | ASK-error + override hint list `--review=rp\|codex\|copilot\|none` | list only the runnable backends (`codex\|copilot\|export\|none`); do not steer to rp |
| plan-review | `skills/flow-next-plan-review/SKILL.md:61` | "**rp** — … Primary backend." glance line | when ineligible, do not present rp as the recommended/primary choice in interactive guidance |
...
- **R4:** In `/flow-next:plan-review`, when `RP_ELIGIBLE=0`, the backend guidance the user sees (ASK-error message `SKILL.md:50-56`, override hints, and the "at a glance" steering) omits `rp` and lists only runnable backends (`codex|copilot|export|none`).
...
- **R8:** No plugin version bump; an `## Unreleased` entry is added to the repo `CHANGELOG.md` (and the docs-site changelog) describing the gate. Docs that describe rp as the "Primary/macOS backend" are checked and, if they imply it is always offered, annotated that non-Mac hosts without rp-cli are not proposed it.
...
- **Out of scope — `flow-next-setup`:** already gates its review-backend menu via `HAVE_RP` + a "macOS only (not detected)" label (`workflow.md:322,466`). Not modified here. (A later cleanup could extract one shared helper across setup/plan/plan-review — noted, not built.)
