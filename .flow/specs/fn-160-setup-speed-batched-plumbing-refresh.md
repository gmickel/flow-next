...
Process note: spec + plan land directly on main (isolated worktree) so the in-flight agent on `chore/review-cap-8` is undisturbed; implementation happens later on its own branch. Sequencing: fn-156 has LANDED (#289, 3.13.3) — that constraint is discharged.

Plan refresh (2026-08-04, post-3.15.0): fn-163/fn-164 landed and REWROTE the exact teaching surfaces this spec's refresh path manages — `templates/usage.md` (fast-path ceremony is now the taught default), the three setup snippets (brief-first cold-session line), and the root CLAUDE.md block. Consequences: (a) the NEW canonical content is the baseline for the `setup.usage_hash` provenance migration and for the R7 question/option inventory — regenerate both baselines from HEAD at implementation time, never from the plan-era copies; (b) quoted flowctl.py anchors (ConfigSnapshot ~:17307, validators ~:17388, setup-block ~:2737) sit BELOW the fn-163/164 insertion regions and are unshifted, but re-verify by grep per standing practice; (c) `flowctl setup` is an EXISTING command group (setup-block etc.) — new verbs join it, no new top-level command; (d) dogfood-parity suites (`test_dogfood_template_parity`, `test_cursor_host_docs`, `test_chart_docs_inventory`) now pin `.flow/usage.md` byte-parity in this repo — refresh fixtures must keep them green.
