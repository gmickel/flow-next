...
- **R28:** Documentation updates land alongside the skill across all surfaces: (a) `CHANGELOG.md` entry; (b) `plugins/flow-next/README.md` — skills/commands table + count incremented + new section explaining the cognitive-aid concept and how to invoke; (c) `CLAUDE.md` commands list updated; (d) `.flow/usage.md` mentions the new command if it lists commands; (e) **public marketing site** at `~/work/mickel.tech/app/apps/flow-next/page.tsx` — append `/flow-next:make-pr` entry to the `commands` array (one-sentence description matching the existing entries' tone), update the lede paragraph that lists features to mention PR-as-cognitive-aid, and add a new FAQ entry (question: "What is PR-as-cognitive-aid?") with a paragraph explaining the structured-input → reviewable-body synthesis (the "don't ask a human to skim 10K lines — ask the agent to make those 10K lines reviewable" framing). The website surface lives in a separate repo (`~/work/mickel.tech`) but is in scope for this epic since the implementing developer (Gordon) is also the maintainer; per `CLAUDE.md` "Contributing / Development" section, external contributors normally skip the website surface, but for this epic the website task is explicitly in scope. [paraphrase]
