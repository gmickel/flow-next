...
**2. NO CONFIG (user decision, 2026-07-03).** There is no `planSync.gate` knob — the gate either proves itself and ships UNCONDITIONALLY (probe-gated whenever `planSync.enabled` is true, with the ramped audit as the built-in safety net), or it does not ship at all. The off|shadow|on enum fn-83.1 shipped is REMOVED in fn-83.4 (config key + its tests). Ramped audit stays as internal mechanics (deterministic ledger-derived: every 2nd skip audited for a repo's first 20 skips, every 5th thereafter; an audit spawn returning `Drift detected: yes` is an **AUDIT MISS** — surfaced loudly in the run summary + ledger). The ledger stays as observability. The ship decision rests on the CROSS-REPO proof below.

**3. CROSS-REPO validation (fn-83.6) — the ship gate.** The fn-83.2 corpus proved safety (0/14 false skips) but exposed 0% skip-rate on THIS repo — flow-next's shared-scope task graph (one flowctl.py, skill dirs referenced everywhere) defeats disjointness proofs and cannot demonstrate yield. The gate's audience is normal feature-sliced repos, so the proof extends to real external flow-managed repos: `~/work/DocIQ-Sphere` (59 specs/239 tasks, multi-stack ~442k LOC), `~/work/gno` (87/217), `~/work/transcribe` (77/175). Per repo: sample ≥8 real completed-task scenarios (task done + downstream todos existed, reconstructed at pinned SHAs; older evidence lacks `base_commit` — derive base as first-evidence-commit^ and document the derivation; unreplayable tasks logged + skipped), generate frozen answer keys the same way (real plan-sync agent, N=3, wobble⇒positive), run the probe. **Ship gate: zero false skips across ALL repos AND aggregate skip-rate on true-negative scenarios ≥50% on the disjoint repos.** Pass ⇒ gate ships always-on; fail ⇒ the gate does NOT ship (probe/ledger/corpus remain as dev assets; wiring reverts to unconditional spawn) — evidence attached either way. **Privacy: external-repo content is NEVER committed to flow-next** — committed artifacts are methodology, scenario pointers (repo path + SHAs + task ids), and aggregate results; raw bodies/keys for external repos live outside the repo (maintainer-local, reproducible by re-running the documented procedure).

**3b. Ground-truth eval harness (`optimization/plan-sync-gate/`) — the safety merge gate (SHIPPED in fn-83.2).**
...
- **R6:** NO user config for the gate: the `planSync.gate` enum + its tests are removed; gate behavior is unconditional (probe-gated whenever `planSync.enabled`) contingent on R14 passing; gate ledger `.flow/plansync-gate.jsonl` always written when the gate runs.
- **R7:** Ramped audit sampling built in: deterministic ledger-derived 1-in-2 for the repo's first 20 skips, 1-in-5 thereafter; AUDIT MISS surfaced in summary + ledger; Ralph path receipt-visible, never prompting.
...
- **R14:** Cross-repo validation (fn-83.6): ≥8 replayed scenarios each from DocIQ-Sphere, gno, transcribe at pinned SHAs with frozen real-agent answer keys; ship gate = zero false skips everywhere AND ≥50% aggregate skip-rate on true negatives from the disjoint repos; external content never committed (methodology + pointers + aggregates only); FAIL ⇒ gate wiring does not ship (unconditional spawn retained, assets kept, evidence attached).
...
| R14 | cross-repo proof (ship gate) | fn-83.6 | — |
