...
- NO `PLAN_DEVIATION` worker terminal line; NO `base_commit` evidence-schema change — both existed only to feed the probe.
...
- **R4:** Gate machinery removed from the shipped plugin: `plan-sync-probe` + probe `_psp_*` (except the retained `_psp_run_git`), `planSync.gate` config, the ledger writer, and the three probe/gate/config tests — all gone; `flowctl anchor` + `test_anchor_bundle.py` intact; no `PLAN_DEVIATION` line, no `base_commit` evidence change remain.
