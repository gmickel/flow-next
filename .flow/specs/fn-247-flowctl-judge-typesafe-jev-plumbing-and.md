...
| `route` | Choice `kind` over the 13 kinds plus `none_of_the_above`; 12 anchored Nouls; the `fork-gate` pair; the `qa-gate` Noul | Lifecycle and direct-route rows decided by code first (the inventory below); only an `intent` or `brief` view reaches the Choice; `kind` at confidence >= 0.7 routes; below the floor or `none_of_the_above` the host decides with the top-3 kinds and probabilities as candidates | Today's host routing |
...
| `fork-gate` | Noul "does the text pose a design or behaviour fork with two named alternatives?"; Choice `observable` / `product_or_preference` / `none_of_the_above` (rides in `route`; standalone preset for routed skills) | Fork present at >= 0.5 else no fork and no question; classification at >= 0.5 else the host decides; `none_of_the_above` goes to the host regardless of confidence | Today's host judgment |
...
**Lifecycle and direct-route inventory, decided in code before any question.** Applied to a `live` view in this order, first match wins, each a fact from `flowctl show --json`, `flowctl spec chain`, and the PR probe:

1. A PR observed for the spec branch -> `existing_pr_tail`. The probe is the one `tail.md` and `auto.md` already use, `gh pr list --head <branch> --state all` (or the tracker bridge), and it keeps their four observations: open, merged (tail work may remain), closed (today's outcome), and probe failed (today's outcome, never `pr_exists=false`). `pr_exists` and `pr_ref` in the state carry the observed value; a failed probe routes as today without a judge call.
2. Tasks exist and all are done -> `all_done_make_pr` (then the QA decision).
3. Tasks exist with an intentional plan, or a sole `implicit_owner` task under `no_plan: true` (the direct route's continuation, including the resume admission the work skill already applies) -> `work_planned` (work on the recorded route).
4. Zero tasks, spec ready -> the plan-versus-no-plan rule in code: a recorded `no_plan: true` selects the direct route outright; otherwise (`false` or absent, which `flowctl show --json` reports identically and neither of which is a recorded plan choice) the `asks_for_plan` / `separate_owners` / `staged_prs` regexes over the spec body (the rule's three positive signals) select `plan`, else `work_no_plan_default`. A read-first research pass is recommended when the research Noul fires and no `## Resolved via Research` section exists.
5. Zero tasks, spec not ready -> the host decides as today between refine, plan-review, and proceeding, with the anchored Nouls printed as hints. Jev's kind is never asked for a live spec: refine and plan-review are the judgment residue fn-246 measured, and the lifecycle rows are where Jev plus code beat a host that routes on input kind.

A `brief` view (a structured brief or spec text arriving at intake, no lifecycle) and an `intent` view reach the kind Choice. Lifecycle kinds are excluded from the Choice's criteria on those views.
...
- The conductor asks `tier` once per task at the dispatch site (work Phase 3c and the rolling scheduler's admission, before the worker is spawned) and at scout dispatch. `mechanical` at the floor makes the conductor spawn the worker on the routing block's fast-scout model through the host's own spawn-model parameter (Claude Code's Agent tool `model`; other hosts their equivalent) and also pass `IMPLEMENTER: <that model>` in the prompt so the worker's existing Phase 1b bridge path resolves the same model when the block names a bridged one; the prompt field alone never selects the model. A host with no spawn-model parameter and no bridge for that model leaves routing unchanged and says so in the line. When the block names no fast-scout model the answer changes nothing. `long_running` at the floor adds a `Tier: long_running (jev 0.86) - bridge recommended` line to the dispatch and the done summary. Everything else is unchanged. An explicit `IMPLEMENTER:` from the invocation is never overwritten. The stage line records the model that actually executed, read back from the worker's return, never the requested one.
...
- **R6:** Flow's route step assembles the route state in code (view, lifecycle fields, the PR probe with `--state all` and its four observations, startable target), applies the lifecycle and direct-route inventory in code for a `live` view (PR tail, all-done, planned or implicit-owner work, ready zero-task direct route where a recorded `no_plan: true` is direct outright and `false` or absent evaluates the plan-versus-no-plan regexes, not-ready to the host), and for an `intent` or `brief` view asks `route` once per hop and routes on `kind` at confidence >= 0.7, otherwise handing the host the top-3 candidates. The `Route:` line above is printed on every hop. Errors: unavailable routes as today with the reason in the line; a failed PR probe routes as today and never reads as no PR; a merged PR with tail work routes to the tail as today; `none_of_the_above` is a fallback, never a route; lifecycle kinds are absent from the Choice on `intent` and `brief` views.
...
- **R11:** The conductor asks `tier` once per task before spawning the worker (work 3c and the rolling scheduler's admission) and at scout dispatch, with the tier state above; `mechanical` at >= 0.8 spawns the worker on the routing block's fast-scout model through the host's spawn-model parameter and passes it as `IMPLEMENTER:` for the bridge path (no change when the block names none, or when the host has neither a spawn-model parameter nor a bridge for it, with the line saying so); `long_running` at >= 0.8 adds the bridge-recommended line; every other answer leaves routing unchanged. The `Tier:` line is written to the dispatch and, with `(model: <what actually ran>)` read from the worker's return, to the task's done summary. Errors: unavailable leaves routing unchanged with the reason in the line; an explicit `IMPLEMENTER:` in the invocation always wins over the tier answer; a worker already spawned is never re-routed; the test asserts the spawn model actually selected, not the prompt line.
...
- **R13:** Unit tests cover R1-R4 and the decision rule of every preset with a stubbed HTTP layer (no network, fixture answers per preset), including the fork `none_of_the_above` case, the four PR-probe observations, and a ready zero-task spec whose record omits `no_plan` (routes direct unless a positive plan signal fires). Each of the six consumers has a stubbed enabled-path integration case that asserts the decision is applied (the clean review counted, the route taken, QA run or skipped with the named half, the fork resolved or handed to the host, the reranked order used, the `IMPLEMENTER:` line passed) and an unavailable case that asserts the fallback path and its stage line. A drift test asserts every kind criterion in the preset equals the route matrix's starting-state and positive-signal cells for its row. The six call sites are exercised by their existing smoke scripts with the key absent (byte-identical to today's output). Errors: none beyond the tests themselves.
...
- Maintainability (plan review): duplication - the kind criteria reproduce the route matrix's starting-state and positive-signal cells inside the preset registry, held equal by a drift test rather than read at runtime (flowctl must not depend on a skill file's location, and the smoke scripts copy flowctl alone); structure - none identified.
