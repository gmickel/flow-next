...
- **R2:** A **collision** is one normalized alias mapping to two **different owners**; the same alias registered twice for the same owner is idempotent and legal. Owner identity is the batch index of the incoming decision. **Aliases that normalize to the empty string are excluded from the check**: today a decision may supply a whitespace-only `id`, which writes `local_map[""]`, and `_normalize_edge_refs` discards empty references so that entry is unreachable and cannot create ambiguity. A registrar that guarded every current assignment would reject two whitespace-only ids and change currently-accepted input. **Non-colliding input behaves exactly as today** - this spec changes no legal behavior on any path.
...
- **R4:** Alias validation applies to the **initial-map path only**, where the namespace is flat because every decision is new. All four write-site classes (`<n>`, `d<n>`, full D-ID, caller-supplied `id`) are validated against each other in both directions. Proven in **both directions and for every generated class**: a caller `id` colliding with a *later* decision's generated alias (the case a fix scoped to `raw["id"]` misses), a *later* caller `id` colliding with an already-registered generated alias (where the generated one is the incumbent), and explicitly including the numeric `<n>` site - without which an implementation could leave `local_map[str(i)]` unregistered and satisfy every other test.
...
- **R10:** Atomicity is about **durable state, not allocation timing**. A genuine canonical full-ID collision can only be detected once a candidate chart id exists, so the guarantee is that a rejection makes **no durable reservation and writes no file** - demonstrated, not asserted: after a rejected initial map no chart files exist and the next valid chart still receives the expected `fn-N`. (R2's "before any D-ID allocation" refers to durable allocation; constructing candidate ids in memory is expected.) Two full-D-ID cases are required and they pull in opposite directions - **a genuine collision against the allocated chart's real full D-ID must be rejected** (decision 1 claims `id: "fn-1.D2"` while decision 2 owns generated `fn-1.D2`), and the provisional sentinel must **not** manufacture a false one. Without the first, an implementation that never registers canonical full-IDs during the real-ID pass passes every other test.
