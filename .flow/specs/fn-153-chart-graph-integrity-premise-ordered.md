...
*R5, R9, R11 and R12 were removed at plan-review round 3 and moved to a follow-up spec: the sharpen alias namespace cannot be fixed without changing public input semantics, which this spec's goal forbids. R-IDs are never renumbered, so the gaps stand.*
...
- **R10:** Atomic rejection is **demonstrated, not asserted**: after a rejected initial map, no chart files exist and the next valid chart still receives the expected `fn-N`.
...
| R5, R9, R11, R12 | Sharpen alias namespace | - | Moved to a follow-up spec: cannot be fixed without changing public input semantics, which this spec's goal forbids |
