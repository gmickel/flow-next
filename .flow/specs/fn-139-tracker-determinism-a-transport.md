...
- **Secrets are injected by the executor, after the adapter boundary.** An adapter never sets an authorization header - if it did, the credential would already have crossed the boundary the redaction rule exists to protect.
- **Credential policy is per request, not global.** `Request.credential_policy` is one of:
  - `provider-auth` (default) - attach the provider credential.
  - **`presigned-anonymous`** - attach **nothing**. Required for Linear's presigned upload `PUT`, which targets a third-party asset host: sending the Linear API key there would leak it to a host that has no business seeing it. An "always inject" executor is a credential-leak path, which is why the policy is explicit rather than inferred.
  - `none` - unauthenticated by design.
- **Cross-host redirect protection.** Credentials are **dropped on any redirect that changes host**, and the executor never silently follows a cross-host redirect while carrying auth.
- **Transport route is per operation, not only per provider.** GitLab is CLI-first for ordinary calls but its **uploads use the HTTP/multipart route**, because `glab api -F file=@` produces invalid multipart (measured). A provider may therefore have both a CLI and an HTTP route; the operation picks.
...
- **TLS**: verification on by default. `sslVerify: false` (per-tracker, or `JIRA_SSL_VERIFY=false`) is honoured **on the HTTP route** for self-hosted internal-CA certs, and is **never silent** - it is recorded on the receipt and in the executor's stderr event, and fails closed when it cannot be recorded. On a **CLI route it is rejected**: `gh`/`glab` expose no TLS flag, and rewriting a CLI call into its HTTP equivalent requires endpoint knowledge that belongs to the adapters, not the transport seam. Providing the HTTP equivalents for CLI-routed operations is adapter work (.4/.6).
- **Transport mechanism is decided per provider AND operation**: GitHub and GitLab use their **CLI** (`gh`, `glab`) for ordinary calls because it already carries host and auth resolution this repo depends on; Linear and Jira use **stdlib HTTP** (`urllib`), having no CLI in the dependency set. **Exception, measured:** GitLab **uploads** use the HTTP/multipart route because `glab api -F file=@` produces invalid multipart. This keeps the zero-dependency rule.
...
- **R4b:** Every request carries an explicit `credential_policy` (`provider-auth` | `presigned-anonymous` | `none`). A presigned third-party upload receives **no** provider credential, and credentials are dropped on any host-changing redirect. Asserted by test: Linear's presigned `PUT` carries only the presigned headers, while its asset **retrieval** does carry auth.
