...
1. **`plugins/flow-next/skills/flow-next-setup/templates/usage.md`** — new `## Orchestration & model steering` section (~40–60 lines; insert after `## Config`, before `## Checkpoint`). **Short *agentic* instructions for driving the other harnesses headlessly** — written for the host agent to execute, distilled from what the project already documents (`codex-delegation.md`, the cursor backend wrappers, review-backend registry) and corrected against current official CLI docs:
...
2. **`plugins/flow-next/skills/flow-next-setup/`** — one new optional ceremony question offering the **orchestration example scaffold**. Slots into the existing single grouped `AskUserQuestion` flow (workflow.md Step 6d, follow its existing grouping/chunking mechanics) with processing in Step 7 **after** the existing Docs-block processing (same target file is touched twice in one run — sequential, re-read from disk, never interleaved). The scaffold content lives in a new **`templates/model-routing-snippet.md`** (single source; sibling of `claude-md-snippet.md`):
   ...
   - **Truth annotations** — reuse the existing `HAVE_CODEX`/`HAVE_CURSOR` probes (workflow.md:359-362); rows/rules for undetected CLIs are written commented-out with "not detected on this machine — uncomment after installing".
   - **Write mechanics** — marker-fenced `<!-- flow-next:model-routing:start -->` / `<!-- flow-next:model-routing:end -->` (colon-namespaced family, matching `flow-next:artifact-link`), with a provenance line ("scaffolded by /flow-next:setup — edit freely; re-run setup to regenerate"). Reuses the Docs-block idiom verbatim: no marker → append; marker present → byte-compare vs canonical → identical = silent no-op (no mtime bump); customized = `Keep mine (Recommended)` / `Overwrite with canonical` / `skip`. Marker presence IS the re-run state — no new config key.
   - **Read-back** — the composed block (with this machine's annotations applied) is shown in full BEFORE writing, options `write` / `skip`; after writing, one confirmation line invites free editing. Never a silent write.
   - **Target file** — the same file(s) the Docs ceremony resolves for this run (platform mapping incl. the existing Droid-with-Claude and Cursor buckets; a "Both" answer writes the same block to both files). If the target's flow-relevant content is a single-line shim pointing at another file, follow the pointer when it resolves in-repo; otherwise report and skip — never turn a shim into a mixed file.
...
- **R3:** The composed block (probes applied) is shown in FULL before writing (options `write`/`skip`); the write lands between explicit start/end markers with a provenance line; a post-write confirmation invites free editing. Never a silent write.
- **R4:** Block content is the full opinionated example — scores table (cost/intelligence/taste) + how-to-apply rules + flow-next surface wiring (worker/`delegate:codex`, review backends incl. per-task `review:`, scouts-may-shell-out-to-cursor-agent, thin-wrapper pattern for gpt-5.5 inside subagents) + graceful-degrade rule — single-sourced in `templates/model-routing-snippet.md`. The escalation rule is always included. Probe-failed CLI rows appear only commented-out with an install note.
...
- **R7:** `docs/orchestration.md` + the flow-next.dev orchestration page gain "in your repo" pointers (usage.md section + setup step); CHANGELOG entry staged under a new `## Unreleased` header (no version bump — batched release per CLAUDE.md).
- **R8:** Tests: ceremony default-skip covered; probe-annotation covered incl. the both-probes-fail path; uninstall marker-removal (incl. damaged-marker no-op) covered; `smoke_test.sh` + full pytest green.
...
- **R10:** Probe-annotation invariant: rules/rows referencing a CLI that failed `command -v` at scaffold time are commented-out with an install note — no silently active route to an unreachable CLI.
- **R11:** Re-run idempotency: marker presence is the state — pristine block = silent no-op (no mtime bump); customized block = `Keep mine (Recommended)` / `Overwrite with canonical` / `skip`; no new config key.
- **R12:** Target-file resolution reuses the Docs ceremony's platform mapping for this run, including "Both" (same block to both files) and the Droid/Cursor buckets; a single-line shim target is followed in-repo or reported + skipped — never converted to a mixed file.
...
| R7  | Docs pointers + CHANGELOG Unreleased | fn-88.4 (repo) | flow-next.dev half lands in that repo at ship time |
