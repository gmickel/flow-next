...
Skill promotion (user decision, 2026-08-28): the plan-time rejection above covered an *always-on* prose skill. The user reversed it for one opportunistic, description-triggered reply skill (`/flow-next:prose`, R7) — scoped to substantial replies, never a pointer or a guarantee — and promoted it from the initially planned experimental tier to stable mid-task ("non-critical, easily adapted if it doesn't work"). The rejection of an always-on skill stands.
