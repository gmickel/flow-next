...
- **R7:** An experimental reply-prose skill ships (`flow-next-prose`, command `/flow-next:prose`, experimental tier): trigger-shaped description scoped to substantial replies/reports/reviews/summaries and explicitly not short turns, tool narration, the visual digest, or artifact output that carries its own pointer; the body is a pointer to `docs/prose.md` (pass identity, never payload) naming the reply-dormant rules (precedence markers/projection; rule 8 softens to lead-with-the-answer) and forbidding rewrites of quoted material, code, or command output. Ships with the command shim, openai.yaml entry, registry-count bumps (inventory only — published counts untouched per the experimental tier), a one-line guide experimental note, and the prose.md scope sentence adjusted (replies governed opportunistically via the skill; visual digest stays excluded). Errors: ambient triggering is host-discretionary and Codex is explicit-invocation-only — a reply the skill never touched is by design, no error surface; doc unreachable → the skill proceeds without it.
