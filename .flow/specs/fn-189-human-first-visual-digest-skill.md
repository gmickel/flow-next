...
- **Command-surface pin.** `test_command_shim_flatten.EXPECTED_COMMANDS` pins the exact shim set - the new `human` shim must be added there deliberately or the suite fails.
...
- R1: Canonical skill exists at `plugins/flow-next/skills/flow-next-human/SKILL.md` with frontmatter `name`, `description`, `allowed-tools`; the description is trigger-rich so natural language invokes it without the slash command ("show me", "explain this visually", "restate that", "digest the plan", "too much text", "walk me through the spec/tasks/diff") and names the four targets (spec, task, diff, current topic). Command shim at `plugins/flow-next/commands/human.md` with bare colon-free `name: human` + non-empty description. Errors: no error surface beyond graceful degradation (missing state -> nearest viable mode, stated in prose).
...
```bash
cd plugins/flow-next/tests && python3 -m unittest test_human_skill test_command_shim_flatten -q
