...
- Comparison table flow-next wins by construction (Bun-style honest table, names competitors): columns = receipts/evidence, cross-model review, live-app QA verdict, tracker projection (4 trackers), autonomy ladder w/ guardrails, multi-harness, self-improving memory, in-repo zero-dep + delete-a-directory uninstall. spec-kit has NO review step at all; Kiro is single-family by construction and charges 5x for spec mode; BMAD is persona theater without verification; conductors are parallelism without a quality spine.
...
- NO fabricated or unlinked testimonials; every quote links to a public source. Replace @dailyreader/@mfeighery/@ben unless links are produced.
...
4. Testimonials rebuilt with linked, named quotes: lead with Novotny (#111, substantive), Patrick Michalina (#5), possibilities (#95), raydocs (#4), Rytis-J (#54), awesome-list feature (#96) with badge. Consent-check the leads.
...
## Open questions (for maintainer critique)

1. Hero line: "Agents generate. flow-next proves." vs keeping "ship like engineers - not slot machines" primary with the proof line as subhead - or a third direction?
2. Comparison content: full named-competitor table (Bun-style, most persuasive, some maintenance) vs capability-only table (safer, weaker)?
3. flow-next.dev theme: commit harder to the single dark-ink/cream identity, or add a true light theme during the makeover?
4. Testimonial outreach: ok to contact Novotny/Michalina/others for permission to feature, or use only what is already public verbatim with links?
5. Enterprise strip wording: comfortable with "field-hardened in enterprise environments" + sector-anonymized stories, or want named-logo permission hunting first?
6. Does the launch moment (post/thread/show-and-tell) ride this spec or follow separately?
