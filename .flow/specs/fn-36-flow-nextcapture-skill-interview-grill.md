...
- **R7:** Phase 4 read-back loop is **mandatory** — agent shows full draft (frontmatter + all sections + R-ID list + `[inferred]` tally) via the platform's blocking-question tool: `AskUserQuestion` on Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if the schema isn't loaded), `request_user_input` on Codex, `ask_user` on Gemini / Droid. Fall back to numbered options in plain text only when no blocking tool is reachable. Never silently skip the read-back. Autofix mode: print full draft + require explicit `--yes` arg to commit (mirrors `memory migrate --yes` pattern).
...
- **R17:** `flow-next-interview/SKILL.md:76-93` extended with lead-with-recommendation pattern: every blocking question (via `AskUserQuestion` on Claude, `request_user_input` on Codex, `ask_user` on Gemini / Droid; fallback to numbered options in plain text when no blocking tool available) includes options summary, recommended option, one-sentence rationale, confidence tier (`[high]` / `[judgment-call]` / `[your-call]`). Reference: `flow-next-audit/SKILL.md:64` for canonical phrasing.
