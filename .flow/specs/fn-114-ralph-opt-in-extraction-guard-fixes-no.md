...
4. Move `flowctl ralph pause/resume/stop/status` (~90 LOC sentinel plumbing) and `find_active_runs` progress.txt parsing (~90 LOC) out of flowctl.py into a repo-local `scripts/ralph/ralphctl.py` (or fold into ralph.sh helpers) installed by ralph-init. `flowctl status` SOFT-PROBES `scripts/ralph/runs/` only when the directory exists (PLAN DECISION 2026-07-21: keeps ralph-user UX, zero cost otherwise). Non-blocking check: glance at flow-next-tui for `flowctl ralph status` / active-runs JSON consumers (maintainer 2026-07-19: low prio - if the move breaks the TUI, that is a follow-up spec, not a blocker here).
