...
describe("getFlowctlPath", () => {
  ...
  it("throws FlowctlNotFoundError when not found", async () => {
    clearFlowctlCache();
    // Point the install-location rungs at an empty home so a real flow-next
    // install on this machine cannot satisfy the search (FLOW_NEXT_TUI_HOME
    // is the test seam in installLocationCandidates).
    process.env.FLOW_NEXT_TUI_HOME = "/tmp/nonexistent-flowctl-test-home";
    // Also isolate PATH: the dev/CI shell may carry a real flowctl (Claude
    // Code injects the plugin bin dir), which would satisfy rung 4 first.
    ...
    try {
      ...
      expect(error instanceof FlowctlNotFoundError).toBe(true);
      const notFoundErr = error as FlowctlNotFoundError;
      expect(notFoundErr.message).toContain("flowctl not found");
      expect(notFoundErr.message).toContain("flow-next plugin");
      ...
      delete process.env.FLOW_NEXT_TUI_HOME;
      clearFlowctlCache();
    }
  });

  it("resolves flowctl from the marketplace install rung via the home seam", async () => {
    clearFlowctlCache();
    const home = await mkdtemp(`${tmpdir()}/flowctl-tui-home-`);
    const marketplace = `${home}/.claude/plugins/marketplaces/flow-next/plugins/flow-next/scripts`;
    await mkdir(marketplace, { recursive: true });
    const fake = `${marketplace}/flowctl`;
    await Bun.write(fake, "#!/bin/sh\nexit 0\n");
    await chmod(fake, 0o755);
    process.env.FLOW_NEXT_TUI_HOME = home;
    const savedPath = process.env.PATH;
    process.env.PATH = "/usr/bin:/bin";
    try {
      const path = await getFlowctlPath("/tmp/nonexistent-flowctl-test-dir");
  ...
  it("prefers the newest semver cache entry regardless of mtime", async () => {
    clearFlowctlCache();
    const home = await mkdtemp(`${tmpdir()}/flowctl-tui-home-`);
    const cacheRoot = `${home}/.claude/plugins/cache/flow-next/flow-next`;
    for (const version of ["2.14.0", "3.33.0"]) {
      const dir = `${cacheRoot}/${version}/scripts`;
      await mkdir(dir, { recursive: true });
      await Bun.write(`${dir}/flowctl`, "#!/bin/sh\nexit 0\n");
      await chmod(`${dir}/flowctl`, 0o755);
    }
    // Touch the STALE entry last so mtime ordering alone would pick it.
    await utimes(`${cacheRoot}/2.14.0`, new Date(), new Date());
    process.env.FLOW_NEXT_TUI_HOME = home;
    const savedPath = process.env.PATH;
    process.env.PATH = "/usr/bin:/bin";
    try {
      const path = await getFlowctlPath("/tmp/nonexistent-flowctl-test-dir");
      expect(path).toBe(`${cacheRoot}/3.33.0/scripts/flowctl`);
    } finally {
      process.env.PATH = savedPath;
      delete process.env.FLOW_NEXT_TUI_HOME;
      clearFlowctlCache();
