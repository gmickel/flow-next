...
2. **Slash command** at `plugins/flow-next/commands/<name>.md` (flat directory - never a `commands/flow-next/` subdir). Frontmatter carries a **bare, colon-free `name: <name>`** (e.g. `name: qa`, NOT `name: flow-next:qa`) plus a non-empty `description`: Claude Code prepends the plugin prefix to the last segment, so a bare name renders `/flow-next:<name>` while a colon re-triples it; Cursor's marketplace review checklist (fn-123 R11) requires both `name` and `description` on every command. Mirror the existing `audit.md` / `prospect.md` shape. Every shim also carries `disable-model-invocation: true`: the command is the user's entry point, so any model-side invocation (a dispatch, hand-off, or "invoke X" instruction in a skill, agent, reference, or setup snippet) names the skill id `flow-next:flow-next-<name>` instead of `/flow-next:<name>`. `sync-codex.sh` rewrites skill ids to `$flow-next-<name>`, and `tests/test_skill_id_invocations.py` resolves every skill-id target to a model-invocable skill.
...
- **A router that lies.** The same class one surface over: the flow route matrix (`/flow-next:flow --explain` reads it) recommending a skill that no longer ships (see the router-staleness rule in `references/route-matrix.md`).
...
1. Run `./scripts/sync-codex.sh` once and commit the mirror diff with the canonical change; CI's `./scripts/sync-codex.sh --check` enforces freshness. Its validation guards must stay green; new Claude-only phrases (tool dispatches, model-name examples) may need a new transform + hard-fail guard (pattern: the fn-100 Explore-dispatch and scout-tier rules).
...
- **Unattended detection** — a skill that may ask decides "attended or not" from the whole autonomy marker namespace: `FLOW_RALPH`, `REVIEW_RECEIPT_PATH`, `FLOW_AUTONOMOUS=1`, `AUTONOMOUS=1`, a `mode:autonomous` token (canonical list: flow `SKILL.md`, "Autonomy refusal"). Never a Ralph-only check. Code run inline for a caller (an "inline wrapper" such as tracker-sync's) uses the calling stage's autonomy, because the wrapper sees no arguments of its own and `flow --auto` dispatches every stage with `mode:autonomous`. Unattended, never prompt: take the documented deferral (`flowctl sync defer` for tracker conflicts, a typed `NEEDS_HUMAN` elsewhere) and continue. Before writing ask/defer prose, read how the sibling skills already handle it.
