...
The one-PR landing workflow (`/flow-next:land <PR>`). It reads matching specs at the remote head and requires all to be closed, resolves threads and CI, and squash merges under current authorization and GitHub gates. Native stacks merge lowest-first, one layer per invocation. Land keeps no ledger and performs no rebase or post-merge repository write. The configured tracker touchpoint follows confirmed merge, and a terminal `LAND_VERDICT` ends the run.

## Machine marker

The invisible HTML comment `<!-- flow-next:make-pr spec=<spec-id> base=<base-ref> -->` emitted under make-pr's visible footer breadcrumb. It records provenance for readers and consumers; land selects specs by their branch name at the PR head and does not gate on this marker.
