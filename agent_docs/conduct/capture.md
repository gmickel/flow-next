...
- [ ] Phase 6 printed exactly one `Recommended next:` line (reason + alternative where it applies) above the unchanged `Next:` menu; no pipeline-variations rubric text was copied into the closer, and `chart` was never the recommended target.
