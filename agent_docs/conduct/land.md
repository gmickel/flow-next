...
- [ ] Land reads every spec at the full remote PR head and selects by `branch_name == headRefName`. If none match, it selects specs done at the head but absent or not done at the PR base tree, with a task entry whose tree SHA is absent from or different in the base tree. Read recursive git trees; `truncated: true` needs human attention. The selected set must be nonempty and closed. An open selection changes nothing; a failed or incomplete read is not an empty set.
- [ ] Land repairs the PR it is given; only merging needs session merge authorization. Ready conversion of a draft requires authorization and `--repo <owner/repo>`.
...
- [ ] Without human in-session merge authorization, a flow-authorized merge waits `land.patienceMinutes` after the last push; a null push date falls back to the head's earliest check-suite creation time, else committer date. A configured `land.mergeVerdictCommand` runs once only when other gates pass; non-zero, missing, unexecutable, and timed-out commands refuse the merge.
