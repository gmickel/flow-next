...
- [ ] Land reads every spec at the full remote PR head and selects by `branch_name == headRefName`. If none match, it selects specs done at the head but absent or not done at the PR base tree, with at least one task blob, reading both trees from the forge. The selected set must be nonempty and closed. An open selection changes nothing; a failed or incomplete read is not an empty set.
