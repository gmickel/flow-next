...
- [ ] Every path in a file tree and every edge in a call tree traces to something read in the transcript — a task file, the spec, `git diff --name-status` (paths; `--stat` is churn-only — its rename labels are not paths), or code opened this session. One invented path, node, or "for clarity" edge breaks this.
