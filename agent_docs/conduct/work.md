...
- [ ] Each worker return is pointer-shaped — task id, terminal status, summary/evidence paths, commit range, and the review receipt path where the path allows it. A return that restates the summary content the files already carry has broken this.
