...
- [ ] Rungs above `agent-browser` are probed (`command -v`, MCP list, `uname -s`) before being planned around, and on a Cursor host `cursor-ide-browser` is probed by exact server id at least once (catalog omission is not absence); an absent driver degrades to the next rung or a documented limitation rather than ending the pass.
