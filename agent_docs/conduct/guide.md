...
- [ ] The smallest-sufficient matrix carries the no-plan route row — a ready zero-task spec with fully-known near-zero-risk work routes to `/flow-next:work <spec-id> --no-plan`, with the safe-skip keeping parallelizable or design-risky work on `/flow-next:plan` and autonomous loops planning. A recommendation set missing the shipped route has broken this.
