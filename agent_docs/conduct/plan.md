...
- [ ] An empirically answerable fork the plan hinges on — a behavior, a timing, an output the running code can settle — is settled by a throwaway probe with the answer read back, never parked as an open question or put to the user. A plan that asked the user for a fact the machine already held has broken this.
