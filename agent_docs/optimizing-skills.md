...
- **Generators (`capture` / `interview`):** all sections required by the resolved current spec template present? every acceptance
  criterion source-tagged (`[user]`/`[paraphrase]`/`[inferred]`)? **capture saves and summarizes the spec, then offers the saved file for editing; interview retains approval before write** (the shared contract in [`plugins/flow-next/docs/read-back.md`](../plugins/flow-next/docs/read-back.md))?
