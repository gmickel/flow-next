...
[![Flow-next](https://img.shields.io/badge/Flow--next-v1.9.0-green)](CHANGELOG.md)
...
| Cursor | Works via a local plugin (`./scripts/install-cursor.sh`) — skills, commands, and **multi-agent flows** (verified). No plugin card + autocomplete under-list (cosmetic); **Ralph unsupported** (hook-schema mismatch) — [details](plugins/flow-next/docs/platforms.md#cursor-local-plugin) |
