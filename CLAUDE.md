...
Implementation via gpt-5.5: `/flow-next:work <id> delegate:codex` (packaged — consent-gated, host keeps git/review) or a direct `codex exec` bridge.
...
Review, cross-family: `review.backend cursor:composer-2.5`.
...
Reach gpt-5.5 inside a subagent (thin-wrapper): a cheap wrapper writes a self-contained prompt, runs `codex exec` over Bash, returns the digest.
