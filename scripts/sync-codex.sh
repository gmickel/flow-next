...
generate_openai_yaml "flow-next-pilot" "Flow Pilot" "Single-tick autonomous build-loop conductor — one ready spec, one stage per tick (pipeline.chainStages on chains qa into make-pr), terminal PILOT_VERDICT line" "#3B82F6" true
...
DIET = {
    ...
    "flow-next-pilot": "Single-tick autonomous build-loop conductor. Advances one ready spec one stage per tick (pipeline.chainStages on chains qa into make-pr), emits PILOT_VERDICT. Use when asked to pilot a spec.",
