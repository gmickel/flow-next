...
nicknames_for() {
  ...
  case "$name" in
    ...
    why-scout)            echo '["Investigator", "Archaeologist", "Sleuth"]' ;;
    *) echo "" ;;
  esac
}
...
for nf in \
  ...
  sed -i.bak \
    ...
    -e 's|Parked unknowns lean `/flow-next:refine`|Parked unknowns lean `$flow-next-refine`|g' \
    -e 's|product or authority choices lean `/flow-next:refine`|product or authority choices lean `$flow-next-refine`|g' \
    ...
    -e 's|Legal targets are ONLY `/flow-next:refine`, `/flow-next:plan`|Legal targets are ONLY `$flow-next-refine`, `$flow-next-plan`|g' \
    -e 's|Same legal targets (`/flow-next:refine`, `/flow-next:plan`|Same legal targets (`$flow-next-refine`, `$flow-next-plan`|g' \
...
SECTION3C
  ...
  for dispatch_field in \
    "FORBIDDEN: implementation edits outside this task's declared Touches (worker lifecycle writes are exempt: .flow/tmp/, the handover paths above, the receipt flowctl done writes); no force-push; no rebase of the target" \
    "TIMEBOX: <cap> - on expiry write the handover with partial findings and return, never run on" \
    "the worker runs the bridge itself and the conductor never bridges" \
    "The \`TIMEBOX\` line is the worker's cap and never the child's"; do
...
#    Used by flow-next-refine/SKILL.md:221.
text = re.sub(
