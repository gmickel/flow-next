...
- **The worker runs the bridge itself when the implementer tier is reached by CLI.** Step 3c of the work skill routes implementation to the implementer tier, but the only thing it spawned was the in-host worker, and the worker's phases never read that tier. A routing block that pins the implementer to a model reached only by `codex exec` left the two documents disagreeing by omission, and on one live run the worker fanned out session-model scouts to pre-digest the repo for a `codex exec` child that re-read it anyway. `agents/worker.md` gains a "Bridged implementer" section gated on that resolution: the worker anchors and persists the base commit as today, skips Phase 1.5 investigation and any scout fan-out, composes a pointer prompt (task id, spec and task paths, the spec's `## Resolved via Research` section when present, the project instruction file, and the instruction to re-anchor, investigate, implement, and test) followed by the usage guide's long-task brief verbatim, runs the bridge in the foreground from the asserted repo root at the sandbox level that permits commits (codex `danger-full-access`, since `workspace-write` keeps `.git/` read-only, #436), and on return commits anything the child left uncommitted (the sandbox-denied case), reviews `<base>..HEAD`, runs the gates on that range, dispatches review, and runs `flowctl done`. Step 3c now states that the conductor never bridges and that its `TIMEBOX` line is the worker's cap, never copied into the child's brief; the minted-task subagent license narrows on this path to parallel bridge calls on disjoint surfaces, each concurrent child in its own workspace and branch because checkpointing children cannot share one Git index, with research and scouting left to the child. Prose only; the Codex mirror is regenerated, and its 3c now carries the implementer-tier paragraph it had dropped since #352. Follow-up to #436. Thanks to @DanielKillenberger for the report (#431).
