...
- **All three deep-pass fallbacks had drifted too.** `DEEP_PASSES_FALLBACK` is a dict rather than a flat constant, so it sat outside the `PARITY_PAIRS` guard and each entry had been compressed to roughly 40% of its `deep-passes.md` body - the adversarial pass lost its four numbered techniques, security and performance lost their focus-area lists and output-format rules. Unlike the validator loader, this one resolves correctly, and the Codex mirror ships `deep-passes.md`, so the fallback only fires on genuinely stripped installs. Fallbacks resynced byte-identical, and the parity guard extended to cover the dict shape plus a check that every declared pass has an entry.
