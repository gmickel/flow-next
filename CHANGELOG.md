...
- New `tests/test_memory_during_spec_null_safe.py` — 12 cases covering the fallback chain: spec wins when present, earliest-task fallback when spec null + tasks have timestamps, branch first-commit fallback via a synthetic git repo (pinned `GIT_COMMITTER_DATE` for determinism), no-signal return-all preserves the graceful-degradation contract, empty-string task entries filtered safely, invalid branch falls through cleanly, missing memory dir returns empty structure.
