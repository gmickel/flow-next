...
- **`pipeline.qa=auto` now takes effect unattended.** At all-done, when the acceptance describes UI behaviour on a drivable surface and a target can be started, QA runs; otherwise the hop records `stage: qa - skipped(config: pipeline.qa=auto: <reason>)` and advances to make-pr. `on` and `off` are byte-for-byte unchanged. QA never blocks the loop.
