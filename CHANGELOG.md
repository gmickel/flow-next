...
- **The bridged child owns the task, including its own fan-out.** When a project's routing block pins the implementer tier to a model the harness reaches only over a CLI bridge, the worker now hands the task to that child instead of silently implementing on the session model: `worker.md` gains a Phase 1b that resolves the tier, skips its own investigation and scouting, composes a pointer prompt (task id, spec and task paths, the resolved-research section when present, the project instruction file) plus the usage guide's long-task brief, runs one foreground bridge call from the asserted repo root at the commit-permitting sandbox, commits any dirty remainder, reviews the child's commit range against the acceptance criteria, runs the focused gates, and continues at its commit phase. The done summary records `stage: implement - ran (model: <what ran>; delegated: <n>)` so a reviewer sees who implemented and whether the child delegated. The work skill's step 3c states that the worker bridges and the conductor never does, and the Codex mirror's 3c carries the implementer-tier paragraph again. The long-task brief's never clause reads "never spawn another bridge" and carries the judicious-subagent license, so a bridged child holds the same delegation license as the in-host worker; the "never spawn another agent" clause that shipped in #436 was a widening no requirement asked for, and PR #437's diagnosis of it (the child could not fan out) is adopted with credit to @DanielKillenberger, while its shape (worker-side parallel bridge calls with per-child worktrees) is not. The July 2026 "keep the child flat" caveats in `flowctl usage` and the codex reach page become dated watch lines naming openai/codex#33267, the measured result (its minimal repro clean 3 of 3 on codex-cli 0.153.4 with gpt-6-astra; zero decode errors across 17 exec-originated spawning runs and 23 spawning child threads in September 2026), and the older-build range where the bug is reported. fn-98's undone R2 is folded in: the codex reach page and the platforms note state that `spawn_agent` model and effort steering works since codex-cli 0.146.0, that a role's `sandbox_mode` is not enforced, the two dispatch gotchas, and the model-selection precedence, in place of "steering is unreliable". STRATEGY.md gains the design principle "The owner holds the license" and `.flow/criteria.md` gains G3, so the next accidental narrowing of an implementer brief is a review finding. No hook, flowctl code, or config key is added; no reach page other than codex changes. Follows #431 and #436.
