...
- **`/flow-next:refine --scope=research`, the read-first pass.** Asks nothing. It dispatches the read-only docs, practice, docs-gap, and memory scouts (github when `scouts.github` is on) over the spec's or task's surface and writes one `## Resolved via Research` section (one sub-block per scout: library versions, changed APIs, gotchas, docs that must change, memory that applies, a source on every line) through refine's existing write-back and read-back. It is observably skipped, with the reason printed and nothing written, when the section already exists or the spec's tasks carry plan's scout findings; `--force` reruns it, and a spec that now names a library the plan never saw reruns it scoped to that delta. Plan's research step applies the same rule in reverse: when the section exists it skips the same scouts (repo-scout, spec-scout, and the gap analyst still run), and when it runs them it writes the section too, so the pass never runs twice. The route matrix's ready-spec row names the read-first signal (the spec names a library or API the repo does not already use) that sends a spec through the pass before work on either route; it never runs by default.
- **`why-scout`, a read-only agent for rationale questions.** It anchors on `git blame` and the PRs behind the commits, reads the tracker thread through the sync bridge or an MCP when one is configured, then the bug and decision memory tracks, and reports each finding with a confidence tier (`direct`, `supported`, `inferred`, `unknown`) the caller may not rewrite. The route matrix's investigation row names it for why questions beside the existing scouts for how questions. Tool-enforced read-only like the other scouts; no new command.

### Changed

- **`/flow-next:interview` is renamed to `/flow-next:refine`.** Same skill, same `business`, `technical`, and `both` scopes and flags; the skill directory is `flow-next-refine` and every canonical surface that names the command now says refine. `/flow-next:interview` stays for this release as a deprecated alias that prints one line and forwards its arguments unchanged (skill-level, non-triggering: `disable-model-invocation: true`, Codex catalog flag off); the alias stub, its command shim, and the sync-roster rows are removed in the release after this one. Published counts are unchanged (the alias is not a stable skill or a new command).
- **Direct execution is the default; plan needs a positive signal.** For a ready spec with no tasks, the route is `/flow-next:work <id> --no-plan` unless the user asked for a plan, separate human owners will implement, delivery is staged across several PRs, or the implementer is routed out of the session model. Risk, size, and file count never trigger plan on their own; design risk routes to plan-review and unresolved product choices route to refine. Capture's and plan's `Recommended next` closers print this rule's result on both the manual and the flow path.
