...
> Canonical example for this repo: [`../../../STRATEGY.md`](https://github.com/gmickel/flow-next/blob/main/STRATEGY.md).
...
- **`/flow-next:refine`** doc-aware mode: surfaces conflicts in a `## Strategy Conflicts` spec section parallel to `## Glossary Conflicts`. Throttle ≤1 strategy-conflict question per interview round.
...
Tier 1 jargon only - Rumelt's "fluff" hallmarks: `synergy / pivot / disrupt / thought-leadership / best-in-class / world-class / 10x`. Two-tier guard: canonical scan in `ci_test.sh` (separate block from R17 - never merge them) covers `flow-next-strategy/SKILL.md` + `cmd_strategy_*` regions in `flowctl.py` + `commands/strategy.md`; mirror scan in [`../../../scripts/sync-codex.sh`](https://github.com/gmickel/flow-next/blob/main/scripts/sync-codex.sh) validation block covers `plugins/flow-next/codex/skills/flow-next-strategy/`. The `references/interview.md` file is excluded - it must describe these anti-patterns to push back on them.

## See also

- [`../../../STRATEGY.md`](https://github.com/gmickel/flow-next/blob/main/STRATEGY.md) - canonical strategy for this repo.
