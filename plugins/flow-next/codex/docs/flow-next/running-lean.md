...
For a ready cohesive spec and a capable coding agent, the recommended route is **spec -> `/flow-next:work <id> --no-plan`**. Add task planning only on a positive signal, where a plan was asked for, separate people implement, or delivery is staged across several PRs. Dependencies, execution constraints, size, and risk alone do not trigger planning. [Pipeline variations](pipeline-variations.md#no-plan-route) separates that choice from refinement and verification; configured gates still apply.
