...
All user-facing skills ship `allow_implicit_invocation: true`, so prose like "plan this feature", "pilot fn-12 to completion", or "open a PR" resolves the matching skill from the model's skill catalog — Codex's naming rule then *requires* it to use that skill. Internal skills that only other skills dispatch (`drive`, `sync`, `export-context`, `worktree-kit`, `deps`) ship `false`: they stay out of the shared skill-catalog context budget (min of 8,000 chars and 2% of the context window, shared with every other skill on your machine) but remain fully invocable by name. Two deliberate user-facing exceptions: `visual` and `prose` also ship `false` — their trigger-rich descriptions are exactly the text the catalog budget exists to keep out, so on Codex both are explicit-only (`$flow-next-visual` / `$flow-next-prose` or the dropdown); the natural-language trigger phrases ("tighten this reply", "apply the prose contract") apply on the hosts that match on skill descriptions (Claude Code, Cursor, Droid, Grok). The mirror's catalog descriptions are dieted to ≤200 chars each for the same reason — the full skill body loads on invocation either way.
