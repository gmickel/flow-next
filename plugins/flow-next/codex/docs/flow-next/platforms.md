...
- [Factory Droid (native support)](#factory-droid-native-support)
- [OpenAI Codex](#openai-codex)
- [Grok Build (Claude Code compatibility)](#grok-build-claude-code-compatibility)
- [Cursor](#cursor)
- [OpenCode](#opencode)
...
`quality-auditor` is review-shaped (a second pair of eyes on uncommitted changes) and stays at `high` - undershooting risks missed regressions. Other intelligent agents do scout/editorial work and run efficiently at `medium`. The worker defaults to `inherit` on BOTH platforms - your session model rules, and flow-next never hardcodes a model opinion into generated config. An OPT-IN pin is available at sync time (`CODEX_MODEL_WORKER` / `CODEX_REASONING_EFFORT_WORKER`) - name a model your own account serves. Steering works (fn-98, codex-cli 0.146.0, verified from the child rollout): a role's declared model and effort apply, an explicit `spawn_agent` parameter applies where the role declares none, `agent_type` takes the role's `name` and needs `fork_turns: "none"`, and a role's `sandbox_mode` is not enforced (read-only stays prompt-only on Codex). The `codex exec -m` self-bridge remains a valid route, not the only one. Reach detail, precedence, and the #33267 fan-out watch line: [`reach/codex.md`](reach/codex.md). The actual review backend (`flowctl <backend> impl-review` / `plan-review` / `completion-review`) is configured separately in `flowctl.py` and defaults on its own to the backend's ranking-top model at `high` effort (current ids in [`flowctl.md`](flowctl.md)).
...
**Instruction files (probe-verified):** Grok loads **both** `CLAUDE.md` and `AGENTS.md` into context (seeded-codename probe, 2026-07-22). Setup therefore writes the lifecycle docs snippet to **CLAUDE.md** by default (`/flow-next:` slash syntax, not Codex `$flow-next-`) and the routing block to **AGENTS.md** (where host-review workflows read the `reviewer` tier). A pre-existing wrong Codex `$flow-next-` marker block is consent-refreshed to the slash form (marker-scoped).
...
What it buys: the Claude-family verdict that, before this backend, only Claude Code could reach through `host` - from Codex, Cursor, Grok Build, Droid or OpenCode as well, with the ladder, the receipt, the round counter and the fix loop of every other CLI backend. Whether that verdict is **independent** is decided by the writer's model family, not by the host:

- **Cross-family** when the session model that wrote the diff is another family - the usual case on Codex and Grok Build, and on Cursor, Droid or OpenCode when a non-Claude session model wrote the diff.
- **Same-family** when a Claude model wrote the diff - always from Claude Code, and from Cursor, Droid or OpenCode whenever the session model is a Claude model. Then the verdict is not the independent one the [reviewer tier](orchestration.md#tiers-what-kind-of-model-a-job-wants) asks for. The backend still runs (a fresh-process Claude second opinion is a legitimate, receipted choice); the receipt records `mode: "claude"` and the model, and the review skills say so once. Prefer `codex` or `host` with a cross-family `reviewer:` pin when independence is the point.
...
| `/flow-next:work` (rolling route, the default scheduler) | **Non-blocking subagent dispatch with completion notifications** (Claude Code's background Task dispatch is the canonical example) | Rolling admission needs the conductor to observe each worker return as it happens. The capability is judged by measurement, never by host name: **Cursor and Grok Build both measured non-blocking and ran the rolling lifecycle end-to-end (2026-08-27; Grok via `spawn_subagent` background mode)**, so all first-class hosts with a subagent primitive currently roll. On a host that genuinely blocks, the run **degrades to wave-shaped dispatch** and records the degradation in its report (`Scheduling: degraded to wave (host lacks non-blocking dispatch)`). Functionality is preserved; only the rolling overlap is lost. |
