...
| `/flow-next:work-rolling` (experimental beta) | **Non-blocking subagent dispatch with completion notifications** (Claude Code's background Task dispatch is the canonical example) | Rolling admission needs the conductor to observe each worker return as it happens. On hosts without that primitive - Cursor, Grok Build, and any host consuming canonical prose without background dispatch + completion notifications - the run **degrades to canonical wave scheduling** and records the degradation in its report (`Scheduling: degraded to wave (host lacks non-blocking dispatch)`). Functionality is preserved; only the rolling overlap is lost. |
