...
|---|---|---|
| [`flow-next-flow`](../../skills/flow-next-flow/SKILL.md) | `/flow-next:flow [<anything>] [--until=merge] [--explain]`, `/flow-next:flow --auto [<spec-id>] [--until=merge] [--tick] [--backlog] [--explain]` | Attended conductor: takes any starting point (nothing, a spec or task id, a branch, a path, a pasted report, a how or why question, a slowness, a cleanup, a design fork, free text; its description names these shapes so a request that names no skill reaches it), routes from its six routing references (`route-matrix`, `spec-count`, `plan-vs-no-plan`, `gate-selection`, `prototype-before-ask`, `tail`; the matrix covers eleven worked variants including refactoring, performance, hill climb, investigation, and prototype), runs the routed stage skill, re-evaluates after each hop, asks a stage's pick inline, and stops only at a decision that ends the run. `--explain` prints the route without running it. Attended flow never runs under Ralph, `flow --auto`, or any autonomy marker. `--auto` is the unattended driver on the same routing reference: one ready spec, hop after hop to a terminal (a PR exists, deferred to land, asked, blocked, needs human, no work), ending with one `PILOT_VERDICT` line; `--tick` runs exactly one hop for a host `/loop` or `/goal`; `--backlog` triages the whole open backlog. `--until=merge` independently selects continuation through land for the selected spec and PR. Default unattended flow stops before merge; attended flow offers landing for an existing PR and asks once unless current scoped authority already exists. See the [tail contract](../../skills/flow-next-flow/references/tail.md) for consent, waits, and recovery. |
...
| [`flow-next-capture`](../../skills/flow-next-capture/SKILL.md) | `/flow-next:capture` | Synthesize the current conversation (or chart briefing) into a spec - source-tagged acceptance criteria (`[user]` / `[paraphrase]` / `[inferred]`), saved-spec summary and editor follow-up. Chart handoff preserves D-ID/evidence links; criterion tags apply only to newly authored bullets. |
| [`flow-next-refine`](../../skills/flow-next-refine/SKILL.md) | `/flow-next:refine` | Refine a spec or task before building (renamed from `interview`; `/flow-next:interview` forwards for one release). Deep Q&A - lead-with-recommendation, confidence tiers, codebase-first investigation; `--scope=business\|technical\|both`. `--scope=research` asks nothing: the read-only docs, practice, docs-gap, and memory scouts write `## Resolved via Research` (one sub-block per scout, a source on every line), skipped when the section or plan's findings already exist, `--force` reruns. |
...
| [`flow-next-plan-review`](../../skills/flow-next-plan-review/SKILL.md) | `/flow-next:plan-review` | Carmack-level cross-model review of a spec or plan (RepoPrompt / Codex / Copilot / Cursor / Claude backend). |
...
| Skill | Trigger | What it does |
|---|---|---|
| [`flow-next-pilot`](../../skills/flow-next-pilot/SKILL.md) | `/flow-next:pilot` | **Deprecated alias** for `/flow-next:flow --auto --tick` (removed next release) - maps `--spec <id>` to the positional id, passes `--backlog`, `--dry-run`, `--review`, `--research`, `--depth` through, prints one deprecation line to stderr, and emits the same `PILOT_VERDICT` line. |
| [`flow-next-land`](../../skills/flow-next-land/SKILL.md) | `/flow-next:land` | Cadence-tick ship loop - babysits build-loop-authored PRs through CI fixes, review convergence, gated explicit merge (merge call interposable via `FLOW_PR_MERGE_CMD` for App/bot-performed merges), spec close, and release-follow; ends with a `LAND_VERDICT` line. Lands chains and GitHub stacks frontier-first: one layer per tick, review verdicts carried across a rebase by patch-id, the layers above a merged parent retargeted by a leased cascade on the plain path, branches deleted only once no open child targets them ([chains-and-stacks](../../skills/flow-next-land/workflow.md#merge-one-layer)). Independently invocable and also consumed one tick at a time by flow's authorized landing stage. |
| [`flow-next-ralph-init`](../../skills/flow-next-ralph-init/SKILL.md) | `/flow-next:ralph-init` | **Deprecated** - prefer `/flow-next:flow --auto` + `/flow-next:land` under a host loop or `cron` ([why](running-lean.md#ralph-deprecated)); existing installs keep working. Scaffold the repo-local Ralph hardened harness under `scripts/ralph/` - external shell loop, fresh session per iteration, hook guardrails, receipts. |
