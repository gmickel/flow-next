...
| [`flow-next-make-pr`](../../skills/flow-next-make-pr/SKILL.md) | `/flow-next:make-pr` | Render a short briefing from the aid artifact and open via `gh` (create call interposable via `FLOW_PR_CREATE_CMD` for App/bot-authored PRs); HTML artifact mode adds an opt-in review lens. |
