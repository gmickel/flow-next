...
1. When a tracker gate resolves to a COMMENT op on a LINKED spec and the host qualifies on the capability ladder below (Tier A or Tier B), dispatch it to a `tracker_runner` subagent - Tier A in the background (keep working), Tier B awaited. Tier C runs it inline.
