...
<!-- probe:grok --> Implementation via grok-4.5: a fast, cheap editing delegate via the grok CLI (flags BEFORE -p: `grok --always-approve -m grok-4.5 --reasoning-effort high -p "<task>"` - blanket approval, trusted repos only; acceptEdits skips Bash and silently truncates shell-using tasks; recipe in `flowctl usage`); host reviews + commits on a taste-heavier tier. Route it to bulk/implementation, NOT UI or final taste-critical work (higher hallucination, weaker on UI). (Or reach grok-4.5 through the cursor review line below.)
