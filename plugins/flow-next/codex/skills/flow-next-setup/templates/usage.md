...
.flow/bin/flowctl config set work.delegate true # /flow-next:work opt-in: offload impl to local `codex exec` (OFF by default, consent-gated; arg `delegate:codex` overrides per-run)
