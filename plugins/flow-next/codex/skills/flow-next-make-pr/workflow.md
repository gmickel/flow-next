...
7. **Review plan** — the exhaustive, bucketed read-order + attention budget (§2.4c): careful → behavior → tests → skim, so the reviewer knows where to spend the next 30 minutes and what is safe to skim. Only when `diff_summary.files[]` has ≥2 files.
8. **Structural changes** — mermaid codefences + prose summary (see §Phase 3).
9. **Decisions made** — `knowledge/decisions/` entries written during the spec.
10. **Memory left behind** — `bug/*` + `knowledge/architecture-patterns/*` entries.
11. **Glossary / strategy notes** — added/renamed terms + tracks served.
12. **Open items** — spec open questions + deferred review findings + spec-completion-review flags.
13. **Live QA** — the `qa_verdict` receipt summary (outcome + open P0/P1 + BLOCKED/NA reason + R-ID coverage), only when the receipt is present (§2.11b).
14. **Where to look** — methodology #4 reviewer-focus list.
15. **Footer breadcrumb** — `Generated by /flow-next:make-pr from <spec-id> against <base-ref> on <YYYY-MM-DD>`.

(The footer is always section "last"; the numbering shifts when optional sections — Not-in-this-PR, Verification, Review plan, Live QA — are omitted, but section *order* is fixed. The three attention constructs are complementary, not redundant: **Critical changes** = the ≤7 highest-risk highlights; **Review plan** = the exhaustive read-order across ALL files with skim-classification; **Where to look** = the per-category reviewer-focus questions.)
...
### 2.4c — Review plan (attention budget)

Render `## Review plan` when `diff_summary.files[]` has ≥2 files — the exhaustive, bucketed READ-ORDER that tells the reviewer where to spend attention and, just as important, what is safe to skim. Where **Critical changes** flags the ≤7 highest-risk highlights, Review plan covers EVERY changed file exactly once, in review order. This is the section that changes *how the reviewer spends the next 30 minutes* rather than only what they know.

Assign each `diff_summary.files[]` path to exactly one bucket by this **deterministic pattern table** — first match wins, and the bucket comes from the pattern, NEVER from a judgment like "this looks mechanical":

| Bucket | Trigger (first match wins) | Reviewer instruction |
|--------|-----------------------------|----------------------|
| 🔴 **Careful** | path ∈ `security_sensitive_paths[]`, OR path ∈ any `public_exports_changed[].file`, OR a source (non-test/doc) path in `high_churn_files[]` | Line-review — the risk lives here |
| 🟠 **Behavior-visible** | path matches `commands/ routes/ pages/ app/ cli/ hooks/ bin/` and not already Careful | Read the behavior change; check the user-facing surface |
| 🟢 **Tests** | path contains `test`, `spec`, `__tests__`, `_test.`, or `.test.` | Verify the assertions match the change; skim the impl |
| ⚪ **Skim** | anything else — `*.md`, `docs/`, `CHANGELOG`, version manifests / lockfiles, or a generated mirror (e.g. a `codex/` sync path) | Skim — mechanical/generated; confirm the generator ran, don't line-review |

- Open with the attention-budget line from `diff_summary.files[]` churn totals: `**Careful-review surface:** ~<sum of additions+deletions over Careful+Behavior files> of <total lines changed> changed lines.`
- Render each bucket's files as GitHub task-list checkboxes (`- [ ]`) in **Careful → Behavior → Tests → Skim** order (PR-body checkboxes persist reviewer progress). Prefix each with the bucket emoji + label once as a sub-heading line.
- **Every `diff_summary.files[]` path appears in exactly one bucket; a path is never invented** (guardrail 1 applies). On large diffs do NOT enumerate every Skim file — group with a count: `- [ ] ⚪ Skim — 12 docs/changelog files (mechanical)`.
...
- Body section order locked (§2.0): H1 title + summary block → TL;DR → R-ID coverage → Critical changes → (Structural changes, §Phase 3) → context sections (§Phase 2 cont) → footer breadcrumb. Sections never reorder.
...
- All 10 hallucination guardrails (§2.5) hold for the rendered output — no fabricated paths (every `<path>` ∈ `diff_summary.files[]`), symbols, or SHAs (every `<sha>` ∈ `tasks[].evidence.commits[]`); every claim traces to a payload field.
...
- `## Where to look` renders questions (not labels) across 5 categories: Architecture / Security / Business correctness / Performance / Tests. Each category's trigger condition references concrete payload signals. Per-category cap (3/3/2/2/1) + section-level cap (8, trimmed in reverse-category order — Architecture never trimmed) enforced. Every focus prompt ends with `?`. Section omitted when no category fires.
