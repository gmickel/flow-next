...
flowctl provides only thin plumbing: `flowctl epic export-cognitive-aid <epic-id> --base <ref> --json` aggregates the inputs into a single JSON payload (Task 1 of this epic). The skill renders, previews via `request_user_input`, pushes, and creates the PR.
