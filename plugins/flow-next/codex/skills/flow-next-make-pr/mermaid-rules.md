...
7. **flowchart: arrow-character preference.** Use `-->` (solid arrow), `-.->` (dashed arrow), `==>` (thick arrow). Avoid ambiguous shapes like `--o` / `--x` unless the connector glyph is intentional — they look like typos in code review.
8. **Total chars ≤12K per codefence.** Count the characters between the opening `​```mermaid` and closing `​````. If above, collapse / group / split.
