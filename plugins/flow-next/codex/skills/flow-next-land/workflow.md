...
The QA stage is advisory to the build loop and `flow --auto` advances a `NEEDS_WORK` QA pass explicitly on the promise that *"the human reviewer + the land gate act on its findings"* (the flow skill's `references/qa-stage.md`). This is that gate — without it, a PR whose latest `qa_verdict` says `NEEDS_WORK` (an open P0/P1 or an uncovered UI-observable R-ID) auto-merges the moment CI is green.
...
|---|---|
| 404 / unparseable / `qa_outcome` ∈ {`SHIP`, `NA`, `BLOCKED`} or any other value | **No QA objection** — continue to 2.6. (`BLOCKED` = "no ship *claim* on a QA basis", not "the app is broken" — advisory, exactly as `flow --auto` treats it; QA never hard-blocks on couldn't-verify.) |
...
if [[ "$REVIEW_SIGNAL" == "silence" && -n "$PATIENCE_AFTER_REVIEW" && "$AUTO_REVIEW_CURRENT" == 1 && "$UNRESOLVED" -eq 0 && -n "$REVIEW_EVENT_AT" ]] \
...
- `CATCHUP_RC != 0` → classify from `CATCHUP_ERR`, mirroring §3.5's stderr split: a merge-conflict refusal (GitHub names the conflict) → verdict `BLOCKED`, reason `base merge conflicts need hand-resolution: <CATCHUP_ERR>` — the conflict verdict is the server's, land still never hand-resolves a hunk. Anything else (already up to date after a race, transient API failure) → verdict `RESOLVING`, evidence line carries `CATCHUP_ERR`, the next tick re-reads `mergeStateStatus` and re-plans — a benign race classified `BLOCKED` would strand the PR behind a verdict no human needs to act on. **Bounded**: record `catch_up_fail` (the error's first line) in the PR's ledger entry on every non-conflict failure and clear it on success; when the SAME first line is already recorded from a previous tick, the failure is not transient (a token without update permission, a fork with maintainer edits off) — apply the §3.4 durable label, verdict `NEEDS_HUMAN`, reason `catch-up failing repeatedly: <CATCHUP_ERR>`. An unattended loop that retried an identical refusal forever has broken this.
...
3. **Tracker touchpoint — the only `Done` driver** — deliberately after release-follow so the verdict comment can carry the release outcome. **`land.merged` is active-by-default whenever the bridge is active**, not gated behind `tracker.perEvent.land.merged != off`. This is deliberate: a real merge is the only event that legitimately projects `Done`, so leaving it opt-in would let boards stick at `In Review` forever after a merge. Like make-pr's unconditional PR-link path, the merge→Done projection rides the bridge-active predicate alone; a run that gated the status on the `land.merged` leaf has broken this (the leaf, if a repo set it, only tunes the optional verdict comment):
   ...
   Verdict-comment wording follows the artifact prose contract in [docs/prose.md](../../docs/flow-next/prose.md) under its "structural contracts win" precedence — the merge-evidence gate and tracker projection rules in this file stay authoritative; proceed without the doc when it is absent.
