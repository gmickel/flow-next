...
AUTHORED=0
# 1) machine marker (make-pr >= 3.11): exact comment node with the spec id
# bound inside it, REQUIRED IN ITS CANONICAL FOOTER POSITION - within the
# last 5 non-empty lines of the body (the marker sits directly under the
# visible breadcrumb; make-pr's §4.6a may append `---` + a tracker ref
# after it, hence a bounded tail window rather than the literal last line).
# The positional bound defeats the fenced-example spoof: a marker quoted
# inside a mid-body code example never sits in the footer tail (its fence
# closer and subsequent sections follow it).
if printf '%s\n' "$PR_BODY" | awk 'NF' | tail -5 \
 | grep -qE "^<!-- flow-next:make-pr spec=${spec}( base=[^ ]+)? -->$"; then
 AUTHORED=1
# 2) legacy anchored footer (pre-marker make-pr): require the FULL canonical
# footer shape on one line - "Generated by ... flow-next:make-pr ... from
# ... <spec-id> ... against ..." in that order (markup-tolerant within the
# line: backticks/italics/link syntax allowed). Three loose substrings are
# NOT enough - prose like "Generated by copying flow-next:make-pr for spec
# <id>" lacks the from/against frame and must not classify.
elif printf '%s' "$PR_BODY" | grep -qE "Generated by[^\\n]*flow-next:make-pr[^\\n]*from[^\\n]*${spec}[^\\n]*against"; then
 AUTHORED=1
fi
