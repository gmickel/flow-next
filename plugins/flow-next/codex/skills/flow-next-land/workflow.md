...
# IN THE FOOTER TAIL, not anywhere in the body, so a mid-body quoted
# breadcrumb or a hand-written "against current behavior" line never
# classifies.
elif printf '%s\n' "$FOOTER_TAIL" \
 | grep -qE "Generated by.*flow-next:make-pr.*from.*${spec}.*against.*on [0-9]{4}-[0-9]{2}-[0-9]{2}"; then
 AUTHORED=1
fi
