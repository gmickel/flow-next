...
**Goal:** build the `## Conversation Evidence` block FIRST. Subsequent phases refer to it by line, not from agent memory of the conversation. This is the audit-trail that makes the source-tagging in Phase 2 verifiable. The evidence is always collected; whether the block is written into the saved spec is the resolved template's call (§2.2).
...
- **Selections count as user input** — a chosen `plain-text numbered prompt` option is recorded as `> user (turn <N>, selected): "<option label verbatim>"`. The label is the evidence; an agent-written gloss of what the choice means is never part of the line.
...
Spec prose follows the artifact prose contract in [docs/prose.md](../../docs/flow-next/prose.md); proceed without it when the doc is absent.
...
The canonical section structure lives in [`plugins/flow-next/templates/spec.md`](../../templates/spec.md) - the single source of truth for the section sequence and per-section ownership annotations (per R17 - never re-embed the section list inline; cross-link the template). At runtime the template is resolved via the 3-tier discovery cascade (first match wins): `<repo_root>/SPEC.md` → `<repo_root>/spec.md` → bundled `${PLUGIN_ROOT}/templates/spec.md`. The bundled file is the canonical source of truth; earlier tiers are user-customized overrides. **The resolved template decides what goes in the spec.** Write the sections it names - as headings or in its `auxiliary_sections` list - in its declared order, follow any instructions it carries, and add no section it leaves out. Draft each section's body using the source-tag conventions below. Position comes from the template too. A section the template places as a heading goes where the template puts it, and an auxiliary section named only in the list takes the placement its list entry states (the bundled entries put `Conversation Evidence`, Phase 1 output verbatim, at the top of the body and `Requirement coverage` at the end). `Requirement coverage` is written on a planned route only (rule below; §2.8 judges the route first). A spec that carries a section its resolved template leaves out has broken this.
...
For a correction supplied in chat, append the user's requirement or rejection verbatim to the evidence first - in the spec's `## Conversation Evidence` block when it has one, otherwise in the Phase 1 evidence still in context. A spec saved without the block never gains one from a correction. To keep the evidence block within the ~30-line cap, trim older evidence using §1.1's truncation marker; never drop the correction just given. Before agent-authored replacement, apply the source-tag checks, Phase 3 material-question checks, and the applicable §5.0 strategy contradiction check to the corrected body, then edit only the affected sections through the normal spec write plumbing.
...
# verbatim from agent context (path-persistence rule: never a shell variable across prompt turns).
