...
if [ "$SPEC_IDS" = "tracker" ] && [ "$BRIDGE_ACTIVE" = "true" ]; then
  ...
  #   SPEC_OUTPUT=$("$FLOWCTL" spec create --tracker-first --tracker-identifier "<KEY|#N|project#iid>" --tracker-id "<durable-id>" --tracker-url "<url>" --title "$SPEC_TITLE" --json)
  #   This writes tracker.identifier, tracker.id, and tracker.url atomically;
  #   tracker.id makes the new spec linked immediately. If the adapter returned
  #   only a key, fetch the issue and use sync set-tracker-id as a compatibility
  #   fallback before seeding the merge base.
  ...
  #   SPEC_OUTPUT=$("$FLOWCTL" spec create --tracker-first --tracker-identifier "$IDENTIFIER" --tracker-id "$ID" --tracker-url "$URL" --title "$SPEC_TITLE" --json)
