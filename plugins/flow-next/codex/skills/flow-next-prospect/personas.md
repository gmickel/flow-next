...
Personas are framing voices. They are *not* parallel subagent dispatch — **Phase 2 generation** runs a single divergent pass inside this skill (no `Task` tool, no `context: fork`), so these persona texts live in the main context. **Phase 3's critique, by contrast, IS a single fresh-context `Task` subagent** (workflow.md §3) — precisely so the personas / focus hint / generation prompt (all here, in the main context) are STRUCTURALLY absent from the critic's window, not merely instructed-absent. The critique subagent sees only the candidate list + grounding snapshot; it is read-only and non-interactive, so the inline-skill rule (keep `plain-text numbered prompt` reachable) is unaffected — the one dispatch carries no question.
