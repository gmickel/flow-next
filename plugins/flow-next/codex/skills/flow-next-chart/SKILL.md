...
description: Decision-map discovery for one oversized unclear idea before capture. Resolve one decision per invocation, brief for capture. Use when asked to chart an idea or work a chart decision.
...
**Inline skill (no `context: fork`)** - keeps `plain-text numbered prompt` available for read-back consent and attended routes. Subagents cannot call plain-text numbered prompts. For read-only scouts use `spawn_agent` with `agent_type: explorer` (or the host's generic read-only dispatch with Edit/Write disallowed when Explore is unavailable). On portable hosts without `plain-text numbered prompt`, fall back to a plain-text numbered prompt with a final `Other - type your own answer` option.
...
2. Ask a plain-text numbered prompt **only** when two interpretations would materially change the chart, cost, or consent boundary.
