...
Use the preset's decision, not a second host classification. The following selector takes the judge JSON (`result`), an explicit invocation model (`explicit_model`, or null), the routing block's fast-scout model (`fast_model`, or null), the host's verified reach (`can_spawn_model`, `can_bridge`), and the model the target role definition pins where the host gives a role's model precedence over the spawn parameter (`role_model`, or null; Codex typed scout roles pin one, see `reach/codex.md`). It returns the dispatch fields and line:
...
if not result["available"]:
    ...
    if value == "mechanical" and not explicit_model:
        if role_model:
            # The role's pinned model wins over a spawn parameter; announce what actually runs.
            tier_line = "Tier: mechanical (jev %.2f) -> %s (role pins model)" % (confidence, role_model)
        elif fast_model and (can_spawn_model or can_bridge):
