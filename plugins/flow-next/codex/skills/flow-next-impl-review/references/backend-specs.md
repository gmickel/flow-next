...
- **codex** — Codex CLI (cross-platform); uses OpenAI models (registry default, see [`flowctl.md`](../../../docs/flow-next/flowctl.md#review-backend)). `FLOW_CODEX_MODEL` / `FLOW_CODEX_EFFORT` env vars, or `--spec codex:gpt-5.4:xhigh`.
- **copilot** — GitHub Copilot CLI (cross-platform); reaches several model families via a Copilot subscription (availability is org-policy managed — a given install may expose fewer). `FLOW_COPILOT_MODEL` / `FLOW_COPILOT_EFFORT` env vars, or `--spec copilot:claude-opus-4.5:xhigh`.
