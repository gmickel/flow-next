...
- **Transports** (fn-52.3 Linear, fn-52.7 GitHub, fn-69 GitLab, fn-70 Jira) implement the interface methods below, each mapping its wire shape to/from the **normalized structs**.
- **Reconcile** (fn-52.4 body, fn-52.5 status/comments) operates ONLY on the normalized structs — it never sees a Linear/GitHub/GitLab/Jira wire detail. This is what makes the 3-way merge transport-blind and testable.
- **Dependency projection** (fn-64) adds two relation methods (`setIssueRelation` / `listIssueRelations`) on the same firewall: the skill resolves `depends_on_epics` edges into (current-issue, dep-issue) pairs and calls the normalized relation transport; each adapter (fn-64.3 Linear, fn-64.4 GitHub, fn-69 GitLab, fn-70 Jira) implements it at its own fidelity. The skill never branches on tracker type (R8).

The structs are the firewall: a transport bug stays in the adapter; a merge bug stays in reconcile.

## Transport interface

Nine methods. Each adapter (Linear via MCP-or-GraphQL; GitHub via `gh`; GitLab via `glab` CLI → raw-REST token fallback; Jira via REST v3/v2 → no-op) implements all nine. The skeleton calls them by name and never branches on tracker type — the active adapter (from `tracker.type`) supplies the implementation.

| Method | Direction | Input | Output | Implemented by |
|---|---|---|---|---|
| `fetchIssue(trackerId)` | tracker → flow | UUID | normalized `issue` (or `not-found` / `errored`) | fn-52.3 / fn-52.7 / fn-69 / fn-70 |
| `writeIssue(issue)` | flow → tracker | normalized `issue` (create if no id, else update) | `{id, identifier, url}` | fn-52.3 / fn-52.7 / fn-69 / fn-70 |
| `listComments(trackerId)` | tracker → flow | UUID | normalized `comment[]` | fn-52.3 / fn-52.7 / fn-69 / fn-70 |
| `postComment(trackerId, body)` | flow → tracker | UUID + markdown body | normalized `comment` | fn-52.3 / fn-52.7 / fn-69 / fn-70 |
| `readStatus(trackerId)` | tracker → flow | UUID | normalized `status` | fn-52.3 / fn-52.7 / fn-69 / fn-70 |
| `setStatus(trackerId, status)` | flow → tracker | UUID + normalized `status` | ok / `errored` | fn-52.3 / fn-52.7 / fn-69 / fn-70 |
| `listIssueRelations(issue)` | tracker → flow | the `issue` to inspect | normalized `relation[]` (or `errored`) | fn-64.3 / fn-64.4 / fn-69 / fn-70 |
| `setIssueRelation(issue, blockedBy)` | flow → tracker | the `issue` + the issue it is **blocked by** | ok / `errored` / `noop` | fn-64.3 / fn-64.4 / fn-69 / fn-70 |
| `listOpenIssues(filter)` | tracker → flow | a `filter` (the promoted-lane `readyState`) | normalized `issue[]` (or `noop` / `errored`) | fn-68.2 (Linear + GitHub) / fn-69 (GitLab) / fn-70 (Jira) |
...
- **Map to/from the normalized structs at the adapter boundary.** Reconcile never receives a wire shape. A new tracker (Notion later) is a new adapter, not a reconcile change.
...
- **No-transport path:** when no transport is reachable (`tracker.type` set but MCP absent AND `LINEAR_API_KEY` absent; or `gh` not authed; or GitLab with no `glab` credential AND no `GITLAB_TOKEN`/`CI_JOB_TOKEN`; or Jira with no `JIRA_BASE_URL` + credential), every method is a documented `noop` + receipt note — never a crash. Same detect-best-available shape as fn-51's driver ladder.
- **Transport is recorded on every receipt** (`--transport mcp|graphql|gh|glab|rest|none`) — `glab` (GitLab `glab` CLI rung) and `rest` (GitLab raw-REST `/api/v4` token fallback **and** the Jira REST `/rest/api/{3,2}` token rung — fn-70) are the token rungs; `none` is the universal no-op rung.
...
```jsonc
{
 "tracker": "linear", // linear | github | gitlab | jira (which adapter produced this)
 "type": "issue", // tracker's item type (issue / ticket)
 "id": "uuid-or-null", // durable id dedupe key (Linear id; GH node id; GitLab global issue id; Jira numeric issue id). null on a create.
 "identifier":"WOR-17", // display key (Linear identifier; GH "#123"; GitLab "<project>#<iid>"; Jira "PROJ-123"). Surfaced in listings.
 "title": "string",
...
```jsonc
{
 ...
 // account ⇒ bot). Jira: `author.accountType` — `app` ⇒ bot, `customer`
 // (JSM portal) ⇒ outsider, `atlassian` (real user) is NOT auto-writer
 // (the comment carries no inline role) ⇒ gate `writer` on a project-role /
 // group-membership check or `tracker.answerAuthors` allowlist, else
 // `unknown` / fail-closed (jira.md § listComments). `unknown` when the
 // transport can't resolve it ⇒ treated as NOT authorized (fail closed).
}
```
...
shared by Linear (MCP + GraphQL), GitHub, GitLab, and Jira; an adapter that detects only the
...
Flow's `depends_on_epics: [B]` on spec A means *A depends on / is blocked by B*. The projected edge is therefore `from = A's issue` (blocked), `to = B's issue` (blocker), `type = "blocks"`. This matches the fn-64.1 ledger's directed `from_tracker_id → to_tracker_id` pair and `setIssueRelation(issue=A, blockedBy=B)`. Adapters translate to their wire form: Linear `issueRelationCreate(type: blocks)` with operands ordered so the blocker blocks the blocked; GitHub `…/issues/{A}/dependencies/blocked_by` pointing at B; GitLab `POST /issues/{A}/links` with `link_type=is_blocked_by` pointing at B (licensed namespace) — on a 403 the fenced block carries the direction; the fenced block lists B as a `#N` "Blocked by" entry under A's body; Jira `POST /rest/api/{3,2}/issueLink` with the configured blocking type, `inwardIssue=B` (the blocker; it blocks), `outwardIssue=A` (the blocked issue) — the universally-available directional link, no fenced-block fallback (jira.md § setIssueRelation). There is no inversion ambiguity — every rung anchors on this one sentence.

### Read-before-write idempotency (mandatory)

**No platform reliably no-ops a duplicate relation**, so every adapter MUST `listIssueRelations(issue)` and check for the (from, to) pair **before** calling `setIssueRelation`. Skip the write when the edge already exists. The Linear dedup must canonicalize across **both** `relations` and `inverseRelations` (the same edge appears from either endpoint) before comparing; the GitHub fenced-block writer must not append a `#N` already present in the block; the GitLab adapter dedups on the **tracker-visible link** — the existing `/links` listing (native `is_blocked_by`, or degraded `relates_to`); the Jira adapter dedups on the **native issue link** — `GET /issue/{A}?fields=issuelinks` filtered to the configured/discovered blocking link type — because Jira creates a *second* link on a duplicate `POST /issueLink` (jira.md § setIssueRelation). The `<!-- flow:deps -->` block GitLab carries on every tier is **provenance, NOT dedup proof**: a block entry whose link is **gone** is a human-removal collision — **queued, default NOT re-created** (`sync defer` autonomous; surface/ask interactive; never silently re-create, R6/R10) — never an already-projected no-op masked by the block. On Jira the equivalent collision is a **ledgered edge whose native link a human deleted** — same defer-on-removal rule, keyed off the ledger vs. the live link (jira.md § setIssueRelation), never the absence of a block (Jira writes none). A re-run over a still-**linked** dependency creates zero new relations and appends nothing (R3).
...
- **Native relations (Linear, GitHub-native deps, GitLab native issue links, Jira native issue links):** provenance lives in the flow-side `depRelations` ledger — `flowctl sync set-dep-relation` records the directed `{key, dep_spec, from_tracker_id, to_tracker_id, type: "blocks", source: "flow", updatedAt}` entry per projected edge (fn-64.1). An edge not in the ledger is, by definition, not ours and is left alone. (Jira writes ONLY native links — no `<!-- flow:deps -->` block — so the ledger is its sole provenance authority; jira.md § setIssueRelation.)
...
| **Implemented by** | fn-68.2 — **Linear** ([linear-graphql.md](linear-graphql.md) / [linear-mcp.md](linear-mcp.md)) + **GitHub** ([github.md](github.md)); **GitLab** ([gitlab.md](gitlab.md)) implements it (fn-69); **Jira** ([jira.md](jira.md)) implements it via JQL (fn-70) |
...
- **Jira** — open issues at the exact `readyState` **Jira status name** (like Linear, Jira has rich per-project workflow states, so readiness is a status NAME used RAW, not a label and not a `statusMap` key) via JQL `project = <KEY> AND status = "<readyState>"` — Cloud `POST /rest/api/3/search/jql` (cursor pagination), DC/Server `GET /rest/api/2/search` (`startAt`). `projectKey` (`^[A-Z][A-Z0-9_]+$` — underscores allowed for DC custom keys, still injection-safe) and `readyState` are sanitized before interpolation — no JQL injection (jira.md § listOpenIssues). No status ordering.
