...
`jq` and a working Python (`python3`, `python`, or `py -3`) must be on PATH. `$FLOWCTL` is required for `memory list` on the `feature-map-drift` tag and `memory upsert` for bug filing. Memory disabled or uninitialised: treat drift search and bug filing as empty, record that in the run notes, continue.
...
**Reporting a product bug.** File to the bug memory track when memory is enabled. Write the body under `$RUN_DIR` first, then file with the full required field set: `"$FLOWCTL" memory upsert --track bug --category <bug category, e.g. runtime-errors|ui|integration> --title "<feature>: <one-line symptom>" --module <surface or component> --body-file <path> --json` (`--title` and `--track` are required - a call without them exits nonzero, and under `set -e` that would end the run without its terminal verdict). Upsert is the deterministic find-or-create (fn-212): a prior entry with exactly this title in the bug track is updated in place, otherwise a new entry is created - no create-then-fold-then-delete dance, no sibling accumulation. Overlap scoring stays on (never `--no-overlap-check`). Invoke it failure-tolerant under `set -e` (e.g. `if _out="$("$FLOWCTL" memory upsert ...)"; then <parse>; else <record in run notes>; fi`): two or more same-titled entries fail closed listing the ids, by design — record that in the run notes instead of guessing, and the nonzero exit must never end the run before its terminal verdict. Parse the JSON with `jq` or python. Memory disabled: record the finding in the run notes only. Either way the finding stays out of the `changed` diff.
