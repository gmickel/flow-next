...
4. **One question per turn**, period. Multi-question turns overwhelm users. Group related options into a SINGLE `request_user_input` (multi-select if non-exclusive); don't queue separate calls.
