...
- **Scout tier: judgment-capable, never a fastest-tier scanner** — mid-tier or stronger (the host's mid-tier), escalating toward the session model's tier when it is stronger or a digest comes back thin. Eval-validated: the fastest tier missed a load-bearing storage-architecture fact that the mid tier found on the identical brief.
