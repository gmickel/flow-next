...
Resolve the scope and capture the hop with one read-only call. Retain `PILOT_SNAPSHOT` as run context across prompt turns; do not persist a config scratch file. An unresolved scope or snapshot error ends `NEEDS_HUMAN` with the error, never a host re-derivation or widened backlog.
