...
| A spec with an existing PR | Apply `tail.md`: attended flow offers landing and obtains current scoped consent; an authorized merge destination invokes `$flow-next-land`; default unattended flow defers | PR exists; a merged PR may have remaining tail work | Review-only convergence keeps its limited scope. PR existence is not consent; land owns all convergence, merge and tail gates | signal absent |
