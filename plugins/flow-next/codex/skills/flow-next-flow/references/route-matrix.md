...
| A structural change with behaviour meant to stay the same (rename, extract, inline, dedupe, move) | Pin the contract first: a characterization test, snapshot, or equivalence check over current behaviour is the R-ID; then `$flow-next-work` and review, migrating callers and deleting the old shape in the same change | No new behaviour named; callers to migrate or a shape to collapse | New behaviour named anywhere makes it a feature with cleanup inside; route to capture or work. Skip the pin only when existing coverage already asserts the contract | signal absent |
...
| One metric to improve against a target through repeated attempts | The hill-climb loop inside `/flow-next:work`: freeze a harness, then one change, one measurement, keep or revert, one commit per kept change, every attempt logged in the task evidence; the target is the R-ID | A harness that reruns cheaply and a target number | One expected fix is the slowness row. Never relax the target to meet it; a plateau is a pivot, a dead end is a report | signal absent |
| A read-only question ("how does X work", "why was Y built this way", "are we sure about Z") | Answer with citations from the repo, git history, and bug and decision memory; no `.flow/` write, no PR. Dispatch the read-only scouts when the surface is wide; `/flow-next:visual` when the answer is a shape | The deliverable is an answer | When the answer is a prerequisite for a change already asked for, route the change and let its stage read. A recommendation between alternatives carries a tradeoffs table | signal absent |
| A design or behaviour fork whose answer is observable | Build a throwaway in a scratch directory, variants behind one switch where alternatives compete, observe on the matching surface, report the decision with its evidence, then route the real build to capture or work | A named decision the prototype exists to make; the output is a decision, not shippable code | Skip when the direction is already set. No decision means no prototype. The rule that sends a fork here instead of to a question is `prototype-before-ask.md` | signal absent |
