...
Route on content and context, never on input kind. A tracker issue, a pasted bug report, console output, a prototype, a branch, a path, and a sentence of intent all become text first: read what was given, then match the starting state. First clear match wins. Each hop re-evaluates; there is no fixed conveyor. Unknown model identity needs no detector and no question: judge the spec and the execution context available.
...
| A ready spec with no tasks and no recorded route | `$flow-next-work <spec-id> --no-plan` (the default), or `$flow-next-plan` only on a positive signal from `plan-vs-no-plan.md` | Ready, cohesive; a capable coding agent can own the whole acceptance contract | Plan on an explicit request, separate human owners, staged multi-PR delivery, or an implementer routed out of the session model. Risk, size, and file count never trigger plan | signal absent |
