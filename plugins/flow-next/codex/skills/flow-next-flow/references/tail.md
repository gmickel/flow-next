...
Flow stops when merge is the only step left and asks before any merge. It never merges on its own, never closes the spec, and never dispatches `/flow-next:land` or a second driver (`/flow-next:flow --auto`). Under `flow --auto` an open PR is deferred to land with the `DEFERRED_TO_LAND` verdict instead of converged; convergence is attended work. Merge and spec close happen only on an explicit instruction inside the run, or through land.
