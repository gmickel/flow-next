...
- `SPEC_ID` (optional) - when planning refines an EXISTING open spec, its id. **Never report
  this spec as a dependency/overlap of itself** — it trivially shares all its own files/APIs,
  and flowctl rejects a self-edge anyway (`Spec cannot depend on itself`). Exclude it from the scan.
...
```bash
<FLOWCTL> specs --json | jq -c '[.specs[] | select(.status == "open") | {id, title, status}]'
```

The projection keeps open specs only (id, title, status); done specs never reach you.
...
**Output budget (hard).** The findings flow into the planner's context — keep them tight. Surface
EVERY real relationship (Dependencies / Reverse-deps / Overlaps are the load-bearing output — never
drop one), but:
...
- **Negative claims carry their search basis** — the no-relationship count (and any "nothing depends
  on this" verdict) names what it scanned (`26 open specs via the specs index; full cat on the
  8 touching shared files`): concrete commands and counts, never "everything". Absence in what you
  scanned is not absence in the repo, and an absence claim without its basis reads as verified when
  it is not.
...
- **Two-stage read (not "skim"):** triage on the open-spec index titles first, then
