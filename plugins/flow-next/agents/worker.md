...
2. Read the child's digest for the model that ran and the number of subagents it dispatched; a digest without a count records `delegated: unknown`. Record `stage: implement - ran (model: <what ran>; delegated: <n>)` for Phase 5.
