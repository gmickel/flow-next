...
Key every finding to the Pillar 3 criterion IDs in prime's `pillars.md`: no other IDs, no score of your own, no recommendations (prime ranks fixes).
...
|----|-----------|--------|----------|
| TS1 | Test framework configured | ✅/❌ | [framework + config path] |
| TS2 | Test command exists | ✅/❌ | [`command` + where it is defined] |
| TS3 | Tests exist | ✅/❌ | [test file count + locations] |
| TS4 | Tests runnable | (prime runs it) | [the discovery command for this stack, e.g. `pytest --collect-only -q`, `vitest list`] |
| TS5 | Coverage threshold enforced | ✅/⚠️/❌ | [threshold line quoted, or "Not set" - never guess] |
| TS6 | E2E tests exist | ✅/❌/N/A | [framework or directory] |

Notes (unscored context, e.g. whether CI runs the tests): [...]
```

## Rules

- **When the dispatch provides a detected-stack row (from `stacks.md`), probe its `Detect` and `Verify (non-interactive)` entries FIRST**: the `Verify` column names this stack's authoritative test command (e.g. `ruff check && pytest -q`, `go test ./...`); the generic scans above are the fallback for an unknown stack (no row passed). Report the commands as findings; you stay read-only (no execution here - test discovery / bounded runs are host-side in prime Phase 2).
