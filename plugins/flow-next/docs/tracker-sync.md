...
| land (merged) | `tracker.perEvent.land.merged` | in-memory status apply (no facade op; the leaf does not gate it) | a PR **merges** (→ issue **Done**, the SOLE Done driver, gated on the GitHub `MERGED` probe; **active-by-default** when bridge active) |
...
records status, relations, and comment in one aggregate receipt. Land does not
call the facade: its `Done` on merge is the in-memory status apply described
under land - Done on merge above, with no comment or receipt.
