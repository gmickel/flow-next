...
| `.flow/charts/<id>-briefing.md` (+ `-briefing-<k>.md`) | Immutable versioned briefing package for capture (B1, B2, ...). A `reopen` stales prior packages; the re-brief that follows advances the counter to `B(n+1)` rather than reinstating a staled B-ID, so the reversal stays in the ledger. |
