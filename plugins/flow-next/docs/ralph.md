...
| `MAX_REVIEW_ITERATIONS` | `8` | Fix+re-review cycles per review. **Enforced deterministically by flowctl (fn-90)** via a cumulative round counter on spec state that survives fresh invocations — at the cap the review command refuses (exit `4` + `ESCALATE:`). See § [Review Loops Until SHIP](#3-review-loops-until-ship). |
