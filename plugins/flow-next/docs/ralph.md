...
**`flow --auto` + land are the default autonomy path** - `/flow-next:flow --auto` builds (ready spec → draft PR, in-session; one invocation drives one item hop after hop to its terminal, and `--tick` runs one hop for a host `/loop` / `/goal` that owns repetition) and `/flow-next:land` ships (draft PR → gated merge and authorized post-merge tail). `flow --auto` classifies each hop from the flow routing reference: [`route-matrix.md`](../skills/flow-next-flow/references/route-matrix.md) names the stage and [`gate-selection.md`](../skills/flow-next-flow/references/gate-selection.md) the review, QA (`pipeline.qa`), and completion-review gates. **Chart is never a stage of `flow --auto`** - optional pre-capture discovery (`/flow-next:chart`) is driven by host `/loop` on the chart skill itself (one unattended D-ID per tick; `CHART_VERDICT=NEEDS_HUMAN` parks attended decisions). `/flow-next:pilot` is the one-release alias for `flow --auto --tick`. Ralph is the **hardened harness** for the work segment: it requires executable tasks, including an already-minted direct owner (its configured loop gates plan-review → work → impl-review → completion review; it never plans, mints the owner, or charts, and direct intent waives no configured review), trades in-session convenience for fresh-session isolation + hook-enforced guardrails, and needs no host loop primitive at all (cron-able on a headless server). Reach for it when a run outlasts a session (`/loop` jobs expire after 7 days) or when prose guardrails aren't enough.

**Optional QA stage (`pipeline.qa`, default off).** `flow --auto` reads the key through the flow skill's [gate-selection reference](../skills/flow-next-flow/references/gate-selection.md), which owns what each value does, and runs one `/flow-next:qa` live pass over the complete build at all-tasks-done, before make-pr, when that reference selects it. QA records runtime findings in the draft PR. CI, staging, and manual QA keep their roles. It never prompts, the gate routes on `qa_outcome` (not the Ralph-guard `verdict` projection), and merge stays the human's plus land's decision.

**Backlog mode (`pilot.autonomy=backlog`, default off).** Enable it with `flowctl config set pilot.autonomy backlog`, or per run with `/flow-next:flow --auto --backlog`. Each run enumerates the open backlog (flow specs via `flowctl ready --all` plus tracker issues at the promoted lane), selects the top dependency-ordered item, triages it, and drives a workable spec through the same pipeline; an item it cannot safely advance is parked behind an async question (`ASKED`) in the spec's `## Open Questions` and a tracker comment, never an interactive prompt. The workflow lives in [`references/backlog-mode.md`](../skills/flow-next-flow/references/backlog-mode.md), read only under `--backlog`; the boundaries (never author a spec, never set `ready`, no merge authority from backlog selection) and the verdict grammar are in the [flow auto workflow](../skills/flow-next-flow/auto.md). With the gate off, `flow --auto` is byte-for-byte unchanged.

| Aspect | Ralph | `flow --auto` |
|--------|-------|-------|
| Scope | spec with executable tasks (planned or direct-owner) → work → reviews (never plans or mints tasks) | ready spec → recorded direct/planned route → work → [opt-in qa] → draft PR → optional authorized land stage (opt-in **backlog mode** widens selection to the whole open backlog: triage → that same span) |
...
Ralph and default `flow --auto` stop before merge. Flow can instead continue through the existing land stage with `flow --auto <spec-id> --until=merge`, under the [scoped consent and recovery contract](../skills/flow-next-flow/references/tail.md). This exception permits only flow invoking one land tick; Ralph nesting restrictions remain. The independently invocable [`/flow-next:land`](../skills/flow-next-land/SKILL.md), babysits those PRs the rest of the way - CI tri-state fix loop, reviewer patience window, resolve-pr convergence, gated explicit merge, spec close, release-follow - and ends each tick with a terminal `LAND_VERDICT` line. Land refuses to nest under the Ralph harness too (same `FLOW_RALPH` / `REVIEW_RECEIPT_PATH` guard).
...
**A pipeline with separate build and landing owners** runs default `flow --auto` and land **concurrently** - `flow --auto` builds spec N while land babysits spec N−1's PR. Same-session (two `/loop` jobs, ticks serialize) works for small backlogs; the real assembly line is **two instances** (Claude Code / Codex), each in **its own clone or git worktree** - both loops mutate the working tree (`flow --auto` checks out spec branches, land checks out PR branches for CI fixes), so two loops sharing one checkout would trip each other's dirty-tree guards. GitHub is the shared state: land pushes the spec close after merging, `flow --auto` pulls the base branch before planning, and the strike ledgers are per-clone by design. Keep scopes disjoint. Default `flow --auto` skips specs with an open PR, but a run with `--until=merge` owns landing for its selected spec and must not compete with standalone land for that item. Land's one-owner-per-clone rule still applies.
...
When QA runs as the **optional `pipeline.qa` stage of `flow --auto`** (default off - see "Optional QA stage" above), the gate reads `qa_outcome` (NOT the Ralph-guard `verdict` projection); what each outcome does next is in the flow skill's [gate-selection reference](../skills/flow-next-flow/references/gate-selection.md). The stage is idempotent - a `head_sha` field on the receipt makes it fresh-iff `id == <spec-id>` AND `head_sha == <spec-branch HEAD>` AND `qa_outcome` is terminal, so `flow --auto` classifies `qa` at most once per branch head and never re-loops. The QA stage **augments, never replaces** CI/staging/manual QA.
