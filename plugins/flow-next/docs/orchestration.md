...
**Known Codex limitation (re-checked Aug 2026):** on GPT-5.6 Sol / Multi-Agent V2 builds, the spawn-layer defects are fixed upstream (openai/codex#32782 `agent_type` and #33268 dropped overrides both closed 2026-07-16), but role-profile application is still not VERIFIABLE (#33314 open) and `codex exec` with MAv2 children still returns undecodable results (#33267 open) - and unverifiable steering fails our routing-evidence bar (read the host record, never the model's self-report). Until those settle, the ROBUST way to steer a different model from a Codex host is the **same-family self-bridge**: `codex exec -m gpt-5.6-terra -c model_reasoning_effort=medium "<self-contained prompt>"` - a fresh process taking `-m` on the command line, immune to the spawn_agent path entirely. Caveats: the child needs process-spawn + network inside the parent sandbox, and keep the child prompt flat (a child that spawns MAv2 subagents of its own can return undecodable results, #33267).
