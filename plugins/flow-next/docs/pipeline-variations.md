...
Both carry the selected spec through its build route and invoke land for PR convergence and gated merge. Make-pr commits the completed spec and final task statuses before opening the PR; the merge carries that close to the base. Plain unattended flow keeps its pre-merge stop. Rerunning plain attended flow on a spec with an existing PR offers landing and asks once unless you have already explicitly authorized that item. Declining or leaving the question unanswered causes no landing mutation; readiness and PR existence are not consent.

Consent covers the selected spec and PR during active retries and waits. A fresh session needs `--until=merge` again or current explicit authority; a historical transcript or receipt does not supply it. Land checks closed specs at the PR head, review threads, CI, GitHub review decision, and the configured merge-verdict command before merging. The configured tracker touchpoint follows merge; release preparation is separate.

Flow passes one PR and the current authorization to land per hop. `--auto --tick` runs at most one landing invocation; a longer run waits at the driver's cadence without spending pilot strikes on external waits. An unrelated PR never replaces the selected target; a missing, ambiguous, or closed-unmerged target stops the route. A confirmed merge ends the run, including when the tracker touchpoint fails. Re-running land on a merged PR repeats only that touchpoint. See the [landing contract](../skills/flow-next-flow/references/tail.md) for consent and continuation. Standalone `/flow-next:land <PR>` uses the same contract.
...
**Signal:** acceptance criteria and material decisions are ready, the work is cohesive, and a capable coding agent can own its implementation. This is the recommended route when task decomposition adds no coordination value. The [GLOSSARY entry](../../../GLOSSARY.md#no-plan-route) names it the **No-plan route**.
