...
5. **Pointer-shaped, not restated.** A handover names *where* the artefact is - spec id, task id, receipt path, commit range - and *what to run next*; it never carries a second copy of the artefact's content. Content in a handover is generated twice, drifts the moment the artefact moves, and is re-read at full length by every consumer; a pointer is stable and the consumer re-reads current truth. Two carve-outs: a consumer **without repo access** gets content because content is the transport there (`/flow-next:export-context`'s external-LLM bundle); and a **bounded control signal** - a verdict enum, an id, a strike class inlined so a transcript-only driver need not re-read the file (`PILOT_VERDICT` / `LAND_VERDICT` / `DELEGATION_*` lines) - is a pointer, not content.
