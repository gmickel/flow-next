...
**Retrieval fix (fn-217):** an entry that is correct and recurring but **not** mechanizable is an Update, not a Keep - the lesson was re-learned while the entry sat unread, so the audit repairs the retrieval surface (`title`, `tags`, `module`, `applies_when`, and placement — a misfiled entry is moved into the category the lesson belongs to, which changes the entry id, so the frontmatter `category` is set to the new bucket and every `related_to` naming the old id is re-pointed in the same edit) and never the body. The report counts these inside Updated as `retrieval fixes`. No new status: there is no read-side signal that would make one truthful.
