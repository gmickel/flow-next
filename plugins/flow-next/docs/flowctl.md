...
| `tracker.enabled` | bool | `false` | Enable the tracker-sync bridge (see [`sync`](#sync)). The bridge is active iff raw `tracker.enabled == true` OR raw `tracker.type ∈ {linear, github, gitlab}`. |
| `tracker.type` | string | `null` | Tracker backend: `linear`, `github`, or `gitlab`. |
...
```bash
# Is the bridge active? (value-checked: enabled OR type ∈ {linear, github, gitlab})
