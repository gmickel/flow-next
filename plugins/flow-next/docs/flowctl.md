...
| `work.delegateModel` | string | `gpt-5.6-sol` | Model passed to `codex exec` for delegated implementation (real work — never a cheaper tier). Hard floor, no fn-76 resolution ladder on this path: requires codex CLI >= 0.144 (older CLIs 400; set `gpt-5.5` until you upgrade). |
| `work.delegateEffort` | string | `medium` | Reasoning-effort floor (`none | low | medium | high | xhigh`). The per-batch risk escalation floors against this; the effort enum includes `none`, not `minimal`. |
