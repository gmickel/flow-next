...
- **`Scheduling: wave (single task | sequential dependency chain)`** - the spec has fewer than two open tasks, or no two open tasks are dependency-independent; one lane has nothing for rolling admission to schedule, so the single-worker wave path runs it with less machinery. Not an error.
