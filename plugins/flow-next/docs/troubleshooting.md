...
- **`Sequential fallback: planSync.enabled=true`** - plan-sync disables concurrent admission entirely (fail-closed); the run degrades to serial, canonical behavior. Expected; turn plan-sync off to get rolling admission.
