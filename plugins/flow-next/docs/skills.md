...
|---|---|---|
| [`flow-next-flow`](../skills/flow-next-flow/SKILL.md) | `/flow-next:flow [<anything>] [--explain]` | Attended conductor: takes any starting point (nothing, a spec or task id, a branch, a path, a pasted report, free text), routes from its six routing references (`route-matrix`, `spec-count`, `plan-vs-no-plan`, `gate-selection`, `prototype-before-ask`, `tail`), runs the routed stage skill, re-evaluates after each hop, and stops at the next human decision. `--explain` prints the route without running it. Never runs under pilot, Ralph, or any autonomy marker. |
