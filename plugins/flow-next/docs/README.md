...
| [`tracker-sync.md`](tracker-sync.md) | `/flow-next:tracker-sync` bridge: projection-not-coordination, discovery ceremony, hybrid id model, sync-state schema, deterministic `flowctl tracker` transport and capabilities, lifecycle touchpoints, Ralph-safe conflict queueing; distinct from `/flow-next:sync` (plan-sync) |
...
- **`/flow-next:interview` now tags the acceptance criteria it writes** - the same `[user]` / `[paraphrase]` / `[inferred]` / `[strategy:<track>]` provenance capture has always emitted, so the grounded-vs-guessed tally and the "re-interview only the `[inferred]` items" pattern finally work on interview-authored specs. `[user]` means the PO under a business pass and the tech lead under a technical one; a pass never retags an existing bullet, and untagged means unknown provenance, never `[user]`. Enable: nothing to configure. Details: [`spec-template.md`](spec-template.md) § Source tags.
- **Review receipts now carry optional versioned structured findings** — supported review routes preserve canonical severity/confidence/classification/status values, durable finding lineage, snapshot-bound anchors, and immutable receipt generations while legacy prose remains valid. Enable: nothing to configure. Details: [`review-findings.md`](review-findings.md).
- **Make PR now preserves one portable, versioned change walkthrough** — supported current artifacts drive GitHub Markdown and optional HTML semantics without mixing stale or legacy fields; the canonical v1 fixture defines byte-pinned downstream vendoring. Enable: nothing to configure. Details: [`pr-cognitive-aid.md`](pr-cognitive-aid.md).
- **Scope markers own the sections your project adds to its spec scaffold** — a section you add via a repo-root `SPEC.md` is now filled and refined by an interview pass when its `<!-- scope: ... -->` marker names that pass's scope, preserved byte-for-byte when it names the other scope, and preserved plus reported in the read-back when it has no marker. Absence from `flowctl scope write-policy` is never permission to drop a section. Enable: add a section to `SPEC.md` with a scope marker; no config. Details: [`spec-template.md`](spec-template.md) § Customizing the scaffold for your project.
- **`tracker.conflictTiebreak` now controls status deadlocks** — `flow-wins` and `tracker-wins` take their documented deterministic paths, while `always-ask` keeps the candidate-bearing conflict. Existing default remains `always-ask`; configure with `flowctl config set tracker.conflictTiebreak flow-wins|tracker-wins|always-ask`. Details: [`tracker-sync.md`](tracker-sync.md) § Reconciliation - who-wins.
