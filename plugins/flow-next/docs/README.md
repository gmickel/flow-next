...
- **5.5.1: your `SPEC.md` decides every section capture writes.** Capture no longer adds `## Conversation Evidence` or `## Requirement coverage` outside the resolved template; it writes the sections the template names, where the template puts them. The `[user]` findability check still runs when the block is left out. Behavior change for a `SPEC.md` written from scratch: with no `auxiliary_sections` list it now gets no auxiliary sections, so copy the list from the bundled template. Enable: delete `Conversation Evidence` from that list to drop the block. See [Leaving a section out](spec-template.md#leaving-a-section-out).

- **5.5.0: maintainability questions at design time; same-actor claims refuse by default.** Plan review and the technical refine pass ask two structural questions from the plan as written (the same edit or decision made in more than one place; a back-edge or new branching in the hottest function) and record concrete findings as advisory, never a gate; the README and this page state what the pipeline does not prove. A second same-actor `flowctl start` on an `in_progress` task now fails with a typed error; confirm the prior run ended and re-run with `--reclaim`. Enable: nothing. Details: [CHANGELOG](../../../CHANGELOG.md).

- **5.4.0: the bridged implementer owns the task.** When the `implementer` tier resolves to a model reached only over a CLI bridge, the worker hands the task to that child (Phase 1b) instead of ignoring the tier; the child commits checkpoints and decides its own delegation under the same license an in-host worker holds, and the worker reviews the child's commit range before review and `flowctl done`. The long-task brief bans only a nested bridge again. Enable: name `implementer: <model> at <effort>` in the routing block, or say it in the moment. See [the bridge route](orchestration.md#implementation-offload-the-bridge-route) and [the Codex reach page](reach/codex.md).
...
- **Optional task decomposition (4.18.0)** - a ready cohesive spec can run through Flow-Next work with its full acceptance contract and configured verification. Use `/flow-next:work <id> --no-plan`; add planning on a positive signal, where a plan was asked for, separate people implement, or delivery is staged across several PRs. Details: [pipeline variations](pipeline-variations.md#no-plan-route).
