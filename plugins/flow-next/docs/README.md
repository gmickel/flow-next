...
| [`../skills/flow-next-pilot/SKILL.md`](../skills/flow-next-pilot/SKILL.md) | `/flow-next:pilot` — single-tick conductor for plan / plan-review / work / make-pr, plus an **optional `qa` stage** (`pipeline.qa==on`, default off, fn-72) at the all-tasks-done juncture before make-pr. Covers `PILOT_VERDICT` grammar, `mode:autonomous` signal, strikes ledger, driver recipes, and Ralph as the alternative driver — never nested. |
