...
- **Structured walkthrough parity.** When a supported current v1 PR cognitive-aid object is available, the HTML lens consumes that exact validated object as the authoritative source for artifact identity/currentness, sources, ordered groups, file membership, separate change/attention dimensions, file-level R-ID/task links, deliberate non-changes, and verification. It may enrich interaction but cannot reclassify, reorder, or mix stale/legacy semantics. See [`pr-cognitive-aid.md`](pr-cognitive-aid.md).
