...
**Step 1 (Research) launches every scout in the depth-appropriate set, in ONE parallel Task call.** The set is the steps.md tier table — the full set at STANDARD/DEEP, at SHORT skip the three web-research scouts and fold docs-gap-scout’s charter into repo-scout. A plan whose research skipped a scout inside its own tier, or ran the set sequentially, has broken this. Each scout in the set provides unique signal.
