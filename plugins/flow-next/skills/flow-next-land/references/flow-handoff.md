...
land_scope_reconcile_rewrite() {
  local base local_ids remote_ids c id idx rc=0
  ...
  # scratch index seeded from the base tree: every local commit absent from the new head is reverse-applied onto it,
  # newest first, so the merged parent's commits unwind in order (a file edited twice by the parent unwinds through its
  # intermediate content). A commit that does not reverse-apply is unpushed work → refuse.
  idx="$(git -C "$REPO_ROOT" rev-parse --git-path flow-next-reconcile.index)"
  GIT_INDEX_FILE="$idx" git -C "$REPO_ROOT" read-tree "$base" 2>/dev/null || { rm -f "$idx"; return 1; }
  local_ids=""
  for c in $(git -C "$REPO_ROOT" rev-list "$(git -C "$REPO_ROOT" merge-base HEAD "$base")..HEAD"); do   # newest first
    id="$(git -C "$REPO_ROOT" show "$c" | git patch-id --stable | cut -d' ' -f1)"; local_ids="$local_ids$id
"
    printf '%s\n' "$remote_ids" | grep -qx "$id" && continue
    git -C "$REPO_ROOT" diff "$c^" "$c" | GIT_INDEX_FILE="$idx" git -C "$REPO_ROOT" apply --cached -R 2>/dev/null || { rc=1; break; }
  done
  rm -f "$idx"; [[ "$rc" == 0 ]] || return 1
