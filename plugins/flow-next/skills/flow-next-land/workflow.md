...
AUTHORED=0
# 1) machine marker (make-pr >= 3.11): exact comment node, spec id bound inside it
if printf '%s' "$PR_BODY" | grep -qE "^<!-- flow-next:make-pr spec=${spec}( base=[^ ]+)? -->$"; then
  AUTHORED=1
# 2) legacy anchored footer: token + spec id must co-occur on the single
#    "Generated by ... /flow-next:make-pr ... from ... <spec-id>" footer line
#    (markup-tolerant within the line: backticks/italics/link syntax allowed),
#    NOT anywhere in the body independently.
