...
#    still lacks the breadcrumb directly above it. (Reproducing the FULL
#    two-line canonical footer verbatim is deliberate forgery, which no
...
if printf '%s' "$MARKER_LINE" | grep -qE "^<!-- flow-next:make-pr spec=${spec}( base=[^ ]+)? -->$" \
   && printf '%s' "$CRUMB_LINE" | grep -q "Generated by"; then
  AUTHORED=1
# 2) legacy anchored footer (pre-marker make-pr): the FULL canonical footer
#    shape - "Generated by ... flow-next:make-pr ... from ... <spec-id> ...
#    against" in order on ONE line (markup-tolerant: backticks/italics/link
#    URLs allowed) - and it must be IN THE FOOTER TAIL, not anywhere in the
#    body, so a mid-body quoted breadcrumb never classifies.
elif printf '%s\n' "$FOOTER_TAIL" \
   | grep -qE "Generated by.*flow-next:make-pr.*from.*${spec}.*against"; then
  AUTHORED=1
fi
