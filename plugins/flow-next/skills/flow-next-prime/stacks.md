...
- **LEG6 atomic pairs:** protobuf generated + source; a dual-copy invariant pair (e.g. a vendored `vendor/lib.py` <-> its upstream `src/lib.py`, a real trap for any contributor's agent when documented only in private notes).
