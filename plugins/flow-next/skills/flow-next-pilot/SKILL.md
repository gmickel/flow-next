...
5. **dispatch** — invoke exactly one stage skill with `mode:autonomous` plus review/research/depth passthroughs (plus the gated `qa`→`make-pr` chain when `pipeline.chainStages` is on). *Done when: one stage skill has been invoked and returned.*
