...
**Glossary term-add consent (only when `GLOSSARY_PROPOSALS` is non-empty AND the user picked `approve`).** One follow-up question via `AskUserQuestion` — the read-back options above stay frozen; this is a separate ask (short — definitions already printed above if present):
...
When `READY_OFFER=true`, one follow-up question via `AskUserQuestion` — the read-back options above stay frozen; this is a separate ask (same shape as the glossary consent):
