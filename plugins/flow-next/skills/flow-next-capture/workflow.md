...
- **Selections count as user input** — a chosen `AskUserQuestion` option is recorded as `> user (turn <N>, selected): "<option label verbatim>"`. The label is the evidence; an agent-written gloss of what the choice means is never part of the line.
