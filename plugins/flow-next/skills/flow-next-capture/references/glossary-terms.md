...
**Glossary term-add consent (only when `GLOSSARY_PROPOSALS` is non-empty AND the user picked `approve`).** One follow-up question via `AskUserQuestion` — the §4.2 read-back options stay frozen; this is a separate ask (short — definitions already printed above if present):
