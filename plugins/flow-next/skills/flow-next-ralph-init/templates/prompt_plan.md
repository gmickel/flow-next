...
- If PLAN_REVIEW=copilot: use `flowctl copilot` wrappers (plan-review with --receipt and `--model "$FLOW_COPILOT_MODEL"`).
...
3) Plan review gate:
  ...
  - If PLAN_REVIEW=copilot: run `/flow-next:plan-review {{EPIC_ID}} --review=copilot` (flowctl will use `--model "$FLOW_COPILOT_MODEL"`).
