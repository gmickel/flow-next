...
- If COMPLETION_REVIEW=copilot: use `flowctl copilot` wrappers (completion-review with --receipt and `--model "$COPILOT_MODEL"`).
...
3) Completion review gate:
  ...
  - If COMPLETION_REVIEW=copilot: run `/flow-next:epic-review {{EPIC_ID}} --review=copilot` (flowctl will use `--model "$COPILOT_MODEL"`).
