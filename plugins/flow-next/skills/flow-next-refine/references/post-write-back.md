...
Applies ONLY when the input was a flow spec (Detect Input Type patterns 1/3) — task ids and file paths carry no spec readiness. Same consent shape and visibility predicate as capture's separate readiness follow-up. Runs after the write-back and the tracker-sync block.
