...
| A ready zero-task spec whose work is fully known, near-zero risk, one implementation context | `/flow-next:work <spec-id> --no-plan` (or answer the zero-task fork's ask with "work directly") | Task decomposition would convert no unknown; the fork mints one implicit task and runs the standard pipeline | Prefer `/flow-next:plan` when independent surfaces could parallelize or design risk remains; autonomous loops keep planning - pilot forwards only an explicit `--no-plan`, and `/flow-next:work-rolling` refuses the route |
