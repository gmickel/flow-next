...
4. **One question per turn**, period. Multi-question turns overwhelm users. Group related options into a SINGLE `AskUserQuestion` (multi-select if non-exclusive); don't queue separate calls.
