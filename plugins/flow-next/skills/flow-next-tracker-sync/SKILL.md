...
| `ready --all` — flow-side open specs + eligibility facts (fn-68) | **`list-open`**: union in the tracker-only items via `listOpenIssues`, no-op when `readyState` unset; **`question`**: author the stable question-valve anchor + post via `postComment`, detect/import the matched answer (steps.md Phase 7) |
...
- **Transport interface** (fn-52.3 Linear, fn-52.7 GitHub implement): `fetchIssue`, `writeIssue`, `listComments`, `postComment`, `readStatus`, `setStatus`, the fn-64 dependency-projection pair `listIssueRelations` / `setIssueRelation`, plus the fn-68 enumeration method `listOpenIssues(filter) → issue[]` (the promoted-lane scan backlog mode unions in — Linear + GitHub in v1). Each maps its wire shape to/from the normalized structs. Defined in [`references/adapter-interface.md`](references/adapter-interface.md).
- **Backlog-mode named ops** (fn-68): `list-open` (enumerate the promoted lane via `listOpenIssues`) and `question <spec-id | tracker-id>` (post the async question-valve comment behind a stable anchor) — skill-level + transport-blind, invoked per-tick by `/flow-next:pilot` backlog mode. Bodies in [steps.md](steps.md) Phase 7; both run under the autonomy gate (never `AskUserQuestion`).
