...
- **Transport interface** (fn-52.3 Linear, fn-52.7 GitHub, fn-69 GitLab, fn-70 Jira implement): `fetchIssue`, `writeIssue`, `listComments`, `postComment`, `readStatus`, `setStatus`, the fn-64 dependency-projection pair `listIssueRelations` / `setIssueRelation`, plus the fn-68 enumeration method `listOpenIssues(filter) → issue[]` (the promoted-lane scan backlog mode unions in — Linear, GitHub, GitLab + Jira). Each maps its wire shape to/from the normalized structs. Defined in [`references/adapter-interface.md`](references/adapter-interface.md).
