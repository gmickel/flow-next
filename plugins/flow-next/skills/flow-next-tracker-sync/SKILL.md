...
| `sync clear` — unlink, wipe state atomically | semantic 3-way body merge (fn-52.4), status who-wins (fn-52.5) |
...
- **Transport interface** (fn-52.3 Linear, fn-52.7 GitHub implement): `fetchIssue`, `writeIssue`, `listComments`, `postComment`, `readStatus`, `setStatus`. Each maps its wire shape to/from the normalized structs. Defined in [`references/adapter-interface.md`](references/adapter-interface.md).
- **Reconcile** operates only on the normalized `issue` / `comment` / `status` structs — never a transport detail. The 3-way **body merge** + format translation + scoped conflict is [`references/body-merge.md`](references/body-merge.md) (fn-52.4); status who-wins + comment append are fn-52.5.
