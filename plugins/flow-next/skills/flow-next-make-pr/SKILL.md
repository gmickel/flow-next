...
1.5b. **Structured PR cognitive aid** — after the optional HTML lens, refresh the final head/merge base and export when needed; then the existing host composes one grounded v1 walkthrough. flowctl validates/persists/selects/renders it before final body creation. No extra model call. Supported current artifacts render compact/full deterministically; stale/invalid artifacts select the existing fallback without mixing fields. The post-creation `makePr` tracker facade remains unchanged.
