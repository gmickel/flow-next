...
description: Render a cognitive-aid PR body from flow-next state and open via gh. Triggers on /flow-next:make-pr with optional epic id and flags (--draft, --ready, --no-mermaid, --base <ref>, --memory, --dry-run). Auto-detects epic from current branch when no id given. NOT Ralph-blocked — autonomous loops can surface a draft PR for human review.
...
A reviewable PR body is itself an artefact: it lets a human decide *where to focus* before skimming the diff. flow-next already collects every input that body needs — epic spec with R-IDs, per-task done summaries and evidence commits, decisions / bug / architecture-patterns memory entries, glossary changes, strategy alignment, deferred review findings, the git diff itself. This skill stitches those into a structured body, optionally adds mermaid diagrams for module-boundary changes, and pushes via `gh pr create`.

The host agent (Claude Code / Codex / Droid) reads the structured payload from `flowctl epic export-cognitive-aid` and synthesizes the body directly. **Every claim in the body must trace to a structured field in the export payload — never fabricate file paths, SHAs, R-ID attributions, or "why" reasoning.** Unknown attribution is honest ("uncovered" / "unclear") rather than invented. The host is competent at "what looks important here?" given the rich input; no second-model review pass is needed (the structured payload does the heavy lifting).

flowctl provides only thin plumbing: `flowctl epic export-cognitive-aid <epic-id> --base <ref> --json` aggregates the inputs into a single JSON payload (Task 1 of this epic). The skill renders, previews via `AskUserQuestion`, pushes, and creates the PR.

**Read [workflow.md](workflow.md) for the full phase-by-phase execution. Read [phases.md](phases.md) for the per-phase Done-when checklists.**
...
**Inline skill (no `context: fork`)** — `AskUserQuestion` must stay reachable across phases. Subagents can't call blocking question tools (Claude Code issues #12890, #34592). Phase 4 preview asks the user `create / dry-run / edit-body / abort` before pushing. (sync-codex.sh rewrites `AskUserQuestion` to `request_user_input` in the Codex mirror.)

## Mode Detection

Parse `$ARGUMENTS` as a flag list. Recognized flags: `--draft`, `--ready`, `--no-mermaid`, `--memory`, `--dry-run`, and `--base <ref>` (consumes the next token). Strip recognized tokens; the remainder (if any) is the optional epic id.
...
| `--memory` | After PR creation, write a `knowledge/architecture-patterns/` memory entry summarizing what shipped. Idempotent — rerun adds no second entry for the same epic id. |
...
- Phase 4's preview is the only mandatory user gate in interactive mode. Phase 0 only asks when something must be resolved (no `--base` and no detection match; tasks not all `done`; no epic detected). Skip questions when context resolves cleanly.
...
When data is missing, the body says so honestly (e.g. `*No decision-track memory entries for this epic. Surface decisions in PR review comments if needed.*`) rather than confabulating content. **Honest "unclear" beats plausible "wrong".**
...
- **Writing memory entries without `--memory`.** Default off. The user opts in for structurally-significant epics — every-PR memory inflation is the failure mode this gate prevents.
...
0. **Pre-flight** — `gh` installed + authenticated; resolve epic id (arg or branch-match); base-branch detection cascade; branch validity (HEAD ahead of base); all tasks `done` (warn-prompt if not); existing-PR refusal filtered on `.state == "OPEN"`. Detects Ralph environment for downstream phases.
1. **Gather inputs** — single call to `flowctl epic export-cognitive-aid <epic-id> --base <ref> --json`; parse the structured payload (epic / tasks / memory / glossary / strategy / diff / reviews).
...
3. **Mermaid generation** — gated by trigger conditions (module boundaries, public interface changes, new top-level dirs, high fan-out epic). Hard caps: 3 diagrams, 12 nodes, 25 edges, 12K characters. Skipped under `--no-mermaid` or when no triggers fire.
...
5. **Output + footer** — emit PR URL on success; print breadcrumb (`Generated by /flow-next:make-pr from <epic-id> against <base>`); optionally write `knowledge/architecture-patterns/` memory entry under `--memory`.
