...
Stop at the first of: the PR exists (run from intent), merge is the only step left (open-PR run), a human decision surfaced by a stage, or a blocking question this hop must ask. Print the report shape from SKILL.md with one `stage:` line per stage reached, then the `Next:` line in the host's command form.
