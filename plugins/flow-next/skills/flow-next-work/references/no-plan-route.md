...
invocation is the only thing that lets an autonomous run take the Direct route. A run
that asked or continued under autonomy without that instruction has broken this.
...
Touches and the minted task declares NONE, so the field renders OMITTED — no path ban;
