...
The reviewer model only sees selected files. RepoPrompt's Builder discovers context you'd miss (rp backend). Codex/Copilot use context hints from flowctl (codex/cp backends).
...
Format: `{"mode":"copilot","epic":"<id>","verdict":"<verdict>","session_id":"<session_id>","timestamp":"..."}`
