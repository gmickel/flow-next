...
Setup copies nothing into `.flow/`, so uninstall there is marker removal: strip the `<!-- BEGIN FLOW-NEXT -->`...`<!-- END FLOW-NEXT -->` block from CLAUDE.md / AGENTS.md (the "Clean up docs" section below does that) and remove the `setup_version` / `setup_mode` stamps from `.flow/meta.json`.
...
**Always check for Codex agent configs and add if any exist.** On Codex, setup copies flow-next's agent configs into `.codex/agents/`; each starts with `# Auto-generated by sync-codex.sh`. Remove only those, so user-authored agents stay:
```bash
if grep -lq '^# Auto-generated by sync-codex.sh' .codex/agents/*.toml 2>/dev/null; then
  echo "grep -l '^# Auto-generated by sync-codex.sh' .codex/agents/*.toml | xargs rm -f"
fi
```
