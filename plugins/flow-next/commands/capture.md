...
Pass the arguments to the skill verbatim. The skill handles mode detection (`mode:autofix` token + `--rewrite <id>` + `--from-compacted-ok` + `--yes`), pre-flight checks (duplicate / relevant evidence completeness after compaction / idempotency), conversation evidence extraction, source-tagged synthesis, must-ask cases, saved-spec review loop, and the flowctl `spec create --plan-file` write.
