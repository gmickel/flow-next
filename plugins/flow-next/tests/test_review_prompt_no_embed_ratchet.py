...
class TestIdentityPathsArePosix(unittest.TestCase):
    """fn-169 — every path in a prompt uses forward slashes, on every platform.

    Caught by CI on windows-latest, not locally: `str(Path.relative_to(...))` is
    platform-dependent, so the prompt rendered `.flow\\specs\\fn-1.md` while
    `<changed_files>` — generated by `git diff --numstat` — stayed forward-slashed.
    Two incompatible path styles in one prompt, and the reviewer is told to run git
    commands against them. git speaks forward slashes on every platform, so POSIX
    is the only correct rendering.
    """

    def test_no_backslash_separators_in_any_identity_slot(self):
        prompt = flowctl.build_review_prompt(
            "impl", context_hints="hints", review_scope=SCOPE,
            diff_range=RANGE, spec_path=".flow/tasks/fn-1.1.md",
        )
        completion = flowctl.build_completion_review_prompt(
            ".flow/specs/fn-1.md", (".flow/tasks/fn-1.1.md",), SCOPE, RANGE,
        )
        for name, text in (("impl", prompt), ("completion", completion)):
            with self.subTest(prompt=name):
                for slot in ("<spec>", "<task_specs>", "<changed_files>"):
                    if slot not in text:
                        continue
                    body = text.split(slot, 1)[1].split("</", 1)[0]
                    self.assertNotIn(
                        "\\", body,
                        f"{name} {slot} contains a backslash separator; git and "
                        "<changed_files> both use forward slashes",
                    )

    def test_handlers_build_paths_with_as_posix(self):
        """Structural: the platform-dependent construction must not come back.

        `str(p.relative_to(root))` reads as harmless and is correct on the machine
        it was written on, which is exactly why it survived local review. Pinned at
        the source so the next identity path added cannot reintroduce it.
        """
        source = (Path(__file__).resolve().parents[1]
                  / "scripts" / "flowctl.py").read_text(encoding="utf-8")
        for handler in ("_backend_impl_review", "_backend_plan_review",
                        "_backend_completion_review", "_load_epic_and_task_specs"):
            start = source.index(f"def {handler}(")
            nxt = source.find("\ndef ", start + 1)
            body = source[start:nxt if nxt != -1 else len(source)]
            with self.subTest(handler=handler):
                self.assertNotIn(
                    "str(epic_spec_path.relative_to", body,
                    f"{handler}: use .as_posix(), not str()",
                )
                self.assertNotIn(
                    "str(task_spec_path.relative_to", body,
                    f"{handler}: use .as_posix(), not str()",
                )
