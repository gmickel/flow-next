...
A ready cohesive spec should run through `/flow-next:work <id> --no-plan` with a capable coding agent when decomposition adds no coordination value. Planning remains useful for dependencies, separate ownership, staged delivery and execution constraints. Material product or authority choices need refinement; implementation details can remain with the owner. Risk and file count determine relevant verification, not whether tasks must be decomposed. Explicit spec/design review works before tasks exist; configured implementation review, acceptance coverage, completion-review policy and opt-in QA remain unchanged. `/flow-next:flow` is the one dial on this opinionated default path. Attended, it reads whatever the user has, routes from one shared routing reference, runs the route, and stops at the next decision that is the user's; `flow --auto` runs the same reference unattended, hop after hop to a draft PR, stopping only where a human is needed. One judgment, two entry shapes; `/flow-next:pilot` is the one-release alias for `flow --auto --tick`.
...
- **The artifact is the contract.** Nothing between the plan and an executor restates content that lives in a file - prompts carry pointers and rails; every executor (native worker, bridged CLI, scout, runner) reads the spec/task files as its brief. Quality budget therefore concentrates in the spec and its acceptance contract, whether captured directly or refined during planning; a thin artifact is refused downstream (the worker's thin-task valve), never compensated for. *(Anchor: fn-103 / decision record `composed-brief-deleted-path-handoff-2026-07-19` - an 8-run eval deleted the composed delegation brief.)*
...
- **The owner holds the license.** Whoever implements owns delegation: the in-host worker on the standard path, the bridged child when the implementer tier reaches a model over a CLI bridge. Wrappers, scouts, and conductors never fan out on the implementer's behalf, because delegation by someone with no stake in the result is not ownership. Safety rules bound push, history rewrite, scope, verdict, and nested bridges, never the owner's own delegation; a never-list that grows past those five has narrowed the owner without a requirement and is a review finding (`.flow/criteria.md` G3). *(Anchors: #436, where "never spawns a bridge of its own" widened to "never spawn another agent or bridge" through three checks; the September 2026 no-plan owner screens, which tied the direct route's quality to one owner seeing the whole task end to end.)*
...
Counts and versions below are dated observations, not current inventory. Read
the manifests and CI workflow for live counts. Track descriptions preserve
product intent; historical rollout detail is labeled separately from current
...
- **Slash-command count and density.** Each command is a discrete handover. 23 commands / 28 skills / 21 subagents at 3.12.0; 28 commands / 32 skills / 20 subagents at the flow release (flow replaced guide, so the command count stayed flat); 27 commands / 31 skills / 21 subagents at the `flow --auto` release (pilot folded into flow as `--auto`, the why-scout joined; the registry inventory also carries the one-release `interview` and `pilot` alias stubs, so it reads 27 commands / 33 skills until the stubs are removed; their command shims are gone); additions must justify a new handover, not a convenience alias (the retired `epic-review` alias was removed entirely in 3.3.1).
...
Historical roadmap snapshot through August 2026; the open items below are not
revalidated commitments or a new priority ordering. Check current specs and
downstream state before scheduling them.
...
- Localization of CLI strings or skill workflow text. English-only; the original through-1.x wording was a historical horizon, not a new localization commitment.
